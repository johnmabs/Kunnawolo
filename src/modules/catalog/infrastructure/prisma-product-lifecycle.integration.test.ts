...
if (databaseUrl === undefined)
  throw new Error(
    "DATABASE_URL is required for product lifecycle integration tests.",
  );
const prisma = createPrismaClient(databaseUrl);
const organizationId = "lifecycle-integration-org";
const productId = "lifecycle-integration-product";
beforeAll(async () => {
  await prisma.organization.upsert({
    where: { id: organizationId },
    create: { id: organizationId, name: "Cycle", currency: "XOF" },
    update: {},
  });
...
describe("Prisma product lifecycle", () => {
  it("keeps lifecycle history and searches Unicode codes only within the organization", async () => {
    ...
    await new SetProductInventoryTracking(products).execute({
      organizationId,
      productId,
    ...
    await expect(
      new SearchProducts(products).execute({ organizationId, query: "kɔd-ɛ" }),
    ).resolves.toMatchObject([
      { id: { value: productId }, trackInventory: false, isActive: true },
    ]);
    await expect(
      prisma.organizationAudit.count({
        where: {
          ...
          action: {
            in: [
              "product.deactivated",
              "product.inventory_tracking_changed",
              "product.activated",
            ],
          },
        },
