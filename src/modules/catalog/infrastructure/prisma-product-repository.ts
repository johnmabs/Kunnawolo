  ...
  public async save(product: Product, audit: ProductAuditEntry): Promise<void> {
    try {
      await this.prisma.$transaction(async (transaction) => {
        ...
        if (
          existing !== null &&
          existing.organizationId !== product.organizationId.value
        ) {
        ...
        if (
          ...
          (await this.prisma.product.findFirst({
            where: {
              organizationId: product.organizationId.value,
              code: product.code,
              NOT: { id: product.id.value },
            },
          })) !== null
        )
          throw new DomainError(
            "catalog.product_code_taken",
            "A product code must be unique within an organization.",
          );
        if (
          ...
          throw new DomainError(
            "catalog.product_barcode_taken",
            "A product barcode must be unique within an organization.",
          );
      }
      throw error;
    }
