...
function toPricing(
  ...
  return ProductPricing.create({
    ...
    productId: Identifier.fromString(row.productId),
    referenceCost: Money.fromMinor(
      Number(row.referenceCostMinor),
      row.currency,
    ),
    salePrice: Money.fromMinor(Number(row.salePriceMinor), row.currency),
  ...
  public async append(pricing: ProductPricing): Promise<void> {
    await this.prisma.$transaction(async (transaction) => {
      const product = await transaction.product.findFirst({
        where: {
          id: pricing.productId.value,
          organizationId: pricing.organizationId.value,
        },
        select: { id: true },
      });
      if (product === null)
      ...
      await transaction.productPrice.create({
        data: {
          id: pricing.id.value,
          organizationId: pricing.organizationId.value,
          productId: pricing.productId.value,
          referenceCostMinor: BigInt(pricing.referenceCost.amountMinor),
          salePriceMinor: BigInt(pricing.salePrice.amountMinor),
          currency: pricing.salePrice.currency,
