...
if (databaseUrl === undefined)
  throw new Error(
    "DATABASE_URL is required for product pricing integration tests.",
  );
const prisma = createPrismaClient(databaseUrl);
const organizationId = "pricing-integration-org";
const otherOrganizationId = "pricing-integration-other-org";
const productId = "pricing-integration-product";
let nextId = 0;
const ids: IdentifierGenerator = {
  next: () => Identifier.fromString(`pricing-integration-${++nextId}`),
};

beforeAll(async () => {
  await prisma.organization.upsert({
    where: { id: organizationId },
    create: { id: organizationId, name: "Tarifs", currency: "XOF" },
    update: {},
  });
  await prisma.organization.upsert({
    where: { id: otherOrganizationId },
    create: { id: otherOrganizationId, name: "Autres tarifs", currency: "XOF" },
    update: {},
  });
  await prisma.product.upsert({
    where: { id: productId },
    create: { id: productId, organizationId, name: "Nsiirin Ɛ Ɔ ɲ ŋ" },
    update: {},
  });
});
afterAll(async () => {
...
describe("PrismaProductPricingRepository", () => {
  it("preserves pricing history, isolation and audit", async () => {
    ...
    const prices = new PrismaProductPricingRepository(prisma);
    const set = new SetProductPricing(products, prices, ids, {
    ...
    await expect(
      ...
      referenceCost: { amountMinor: 600 },
      salePrice: { amountMinor: 900 },
      reference: "Tarif Ɔ",
    });
    await expect(
      prisma.productPrice.count({ where: { organizationId, productId } }),
    ).resolves.toBe(2);
    await expect(
      prisma.organizationAudit.count({
        where: { organizationId, action: "product.pricing_set" },
      }),
    ).resolves.toBe(2);
    await expect(
      new GetCurrentProductPricing(products, prices).execute({
