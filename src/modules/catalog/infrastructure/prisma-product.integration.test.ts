...
const ids: IdentifierGenerator = {
  next: () => Identifier.fromString(productId),
};
...
describe("PrismaProductRepository", () => {
  it("persists, reads, updates and finds exact NFC Unicode values within its organization", async () => {
    ...
    await new CreateProduct(repository, ids).execute({
      organizationId,
      actorId: "actor-1",
    ...
    const created = await new GetProduct(repository).execute({
      organizationId,
      productId,
    });
    ...
    await new UpdateProduct(repository).execute({
      organizationId,
      productId,
      actorId: "actor-2",
      name: created.name,
    ...
    await expect(
      new SearchProducts(repository).execute({
        organizationId,
        query: "nsiirin",
      }),
    ).resolves.toMatchObject([
      { id: Identifier.fromString(productId), isActive: false },
    ]);
    await expect(
      prisma.organizationAudit.findMany({
        where: {
          organizationId,
          action: { in: ["product.created", "product.updated"] },
        },
      }),
    ).resolves.toHaveLength(2);
    await expect(
      new GetProduct(repository).execute({
  ...
  it("enforces code and barcode uniqueness per organization while allowing another organization", async () => {
    const repository = new PrismaProductRepository(prisma);
    const duplicateCode = Product.create(
      Identifier.fromString(otherProductId),
      Identifier.fromString(organizationId),
      { name: "Autre", code: "NSI-Ɛ-2", barcode: "999" },
    );
    await expect(
      repository.save(duplicateCode, {
        organizationId,
        actorId: "actor-3",
        action: "product.created",
      }),
    ).rejects.toMatchObject({ code: "catalog.product_code_taken" });
    const sameCodeElsewhere = Product.create(
      Identifier.fromString(otherProductId),
      Identifier.fromString(otherOrganizationId),
      { name: "Autre", code: "NSI-Ɛ-2", barcode: "002" },
    );
    await expect(
      repository.save(sameCodeElsewhere, {
        organizationId: otherOrganizationId,
        actorId: "actor-3",
        action: "product.created",
      }),
