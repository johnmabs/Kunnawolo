  ...
  public static fromSales(
    ...
    for (const sale of input.sales) {
      if (
        ...
        !Number.isSafeInteger(sale.revenueMinor) ||
        !Number.isSafeInteger(sale.costMinor) ||
        sale.revenueMinor < 0 ||
        sale.costMinor < 0
      )
        throw new DomainError(
          "reporting.invalid_sales_snapshot",
          "A finalized sale snapshot is invalid for reporting.",
        );
    ...
    return new SalesProjection(
      ...
      Money.fromMinor(revenueMinor, input.currency),
      Money.fromMinor(costMinor, input.currency),
      Money.fromMinor(revenueMinor - costMinor, input.currency),
      input.sales.length,
    );
  }
}
