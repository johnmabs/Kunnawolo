...
if (databaseUrl === undefined)
  throw new Error(
    "DATABASE_URL is required for stock projection integration tests.",
  );
...
beforeAll(async () => {
  await prisma.organization.createMany({
    data: [
      { id: organizationId, name: "Stock Ɛ", currency: "XOF" },
  ...
  await prisma.stockLevel.createMany({
    data: [
      {
        id: "stock-projection-level-active",
        organizationId,
        shopId: activeShopId,
        productId,
        quantity: 10,
      ...
      {
        id: "stock-projection-level-inactive",
        organizationId,
      ...
      {
        id: "stock-projection-level-other",
        organizationId: otherOrganizationId,
        shopId: otherShopId,
  ...
  await prisma.stockLoss.create({
    data: {
      id: "stock-projection-loss",
      organizationId,
      shopId: inactiveShopId,
      productId,
      quantity: 2,
  ...
  await prisma.inventoryReconciliation.create({
    data: {
      id: "stock-projection-reconciliation",
      organizationId,
      shopId: inactiveShopId,
      reference: "RAPP-Ɛ",
      ...
      lines: {
        create: {
          id: "stock-projection-anomaly",
          productId,
          stockLevelId: "stock-projection-level-inactive",
          stockLevelQuantity: 5,
          ledgerQuantity: 4,
  ...
  await prisma.stockTransfer.create({
    data: {
      id: "stock-projection-sent",
      organizationId,
      sourceShopId: activeShopId,
      destinationShopId: inactiveShopId,
      status: "SENT",
      shipmentReference: "TRF-SENT",
      sentAt: occurredAt,
      lines: {
        create: { id: "stock-projection-sent-line", productId, quantity: 3 },
      },
    },
  });
  await prisma.stockTransfer.create({
    data: {
      id: "stock-projection-received",
      organizationId,
      sourceShopId: activeShopId,
      destinationShopId: inactiveShopId,
      status: "RECEIVED",
      shipmentReference: "TRF-RECEIVED",
      sentAt: occurredAt,
      receptionReference: "TRF-RECEIVED-IN",
      receivedAt: occurredAt,
      lines: {
        create: {
          id: "stock-projection-received-line",
          productId,
          quantity: 9,
        },
      },
...
afterAll(async () => {
  await prisma.inventoryReconciliationLine.deleteMany({
    where: { id: "stock-projection-anomaly" },
  });
  await prisma.inventoryReconciliation.deleteMany({
    where: { id: "stock-projection-reconciliation" },
  });
  await prisma.stockTransferLine.deleteMany({
    where: {
      id: {
        in: ["stock-projection-sent-line", "stock-projection-received-line"],
      },
    },
  });
  await prisma.stockTransfer.deleteMany({
    where: {
      id: { in: ["stock-projection-sent", "stock-projection-received"] },
    },
  });
  ...
  await prisma.stockLevel.deleteMany({
    where: {
      id: {
        in: [
          "stock-projection-level-active",
          "stock-projection-level-inactive",
          "stock-projection-level-other",
        ],
      },
    },
  });
...
describe("stock projection", () => {
  it("projects isolated stock, transit, losses, and reconciliation anomalies including inactive shops", async () => {
    const project = new ProjectStock(
    ...
    await expect(
      ...
      onHandQuantity: { value: 15 },
      inTransitQuantity: { value: 3 },
      lossQuantity: { value: 2 },
      anomalyCount: 1,
    });
    await expect(
    ...
    await expect(
      ...
      onHandQuantity: { value: 7 },
      inTransitQuantity: { value: 0 },
