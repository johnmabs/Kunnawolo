...
if (databaseUrl === undefined)
  throw new Error(
    "DATABASE_URL is required for report-export integration tests.",
  );
...
beforeAll(async () => {
  await prisma.organization.createMany({
    data: [
      { id: organizationId, name: "Exports Ɛ", currency: "XOF" },
      { id: otherOrganizationId, name: "Autres exports", currency: "XOF" },
    ],
    skipDuplicates: true,
  });
  await prisma.shop.createMany({
    data: [
      { id: shopId, organizationId, code: "CSV-Ɛ", name: "Historique Ɔ" },
      {
        id: otherShopId,
  ...
  await prisma.userAccount.createMany({
    data: [
      {
        id: ownerId,
        email: "report-export-owner@example.test",
        displayName: "Owner",
      },
      {
        id: managerId,
        email: "report-export-manager@example.test",
        displayName: "Manager",
      },
    ],
    skipDuplicates: true,
  ...
  await prisma.organizationMembership.createMany({
    data: [
      {
        id: "report-export-owner-membership",
        organizationId,
      ...
      {
        id: "report-export-manager-membership",
        organizationId,
  ...
  await prisma.shopAssignment.upsert({
    where: {
      membershipId_shopId: {
        membershipId: "report-export-manager-membership",
        shopId,
      },
    },
    create: {
      id: "report-export-manager-assignment",
      membershipId: "report-export-manager-membership",
      shopId,
    },
    update: {},
...
afterAll(async () => {
  ...
  await prisma.shopAssignment.deleteMany({
    where: { id: "report-export-manager-assignment" },
  });
  await prisma.organizationMembership.deleteMany({
    where: {
      id: {
        in: [
          "report-export-owner-membership",
          "report-export-manager-membership",
        ],
      },
    },
  });
...
describe("report export persistence", () => {
  it("authorizes historical shops, isolates organizations, and records an auditable Unicode CSV export", async () => {
    ...
    await expect(
      authorization.authorize(organizationId, shopId, managerId),
    ).resolves.toBeUndefined();
    await expect(
      authorization.authorize(organizationId, null, managerId),
    ).rejects.toMatchObject({ code: "reporting.read_forbidden" });
    await expect(
      authorization.authorize(organizationId, otherShopId, ownerId),
    ).rejects.toMatchObject({ code: "reporting.shop_not_found" });

    const repository = new PrismaReportExportRepository(prisma);
    const reportExport = ReportExport.create({
      id: "report-export-id",
      filter: DashboardFilter.create({
        organizationId,
        shopId,
        occurredFrom: at,
        occurredTo: at,
      ...
      reference: "EXPORT-ɛ",
      actorId: ownerId,
      content: '"metric","Historique Ɔ"',
      exportedAt: at,
    });
    await repository.save(reportExport);

    await expect(
      repository.findByReference(organizationId, "EXPORT-ɛ"),
    ).resolves.toMatchObject({
      content: '"metric","Historique Ɔ"',
      filter: { shopId: { value: shopId } },
    });
    await expect(
      repository.findByReference(otherOrganizationId, "EXPORT-ɛ"),
    ).resolves.toBeNull();
    await expect(
      prisma.organizationAudit.findFirst({
        where: {
          organizationId,
          actorId: ownerId,
          action: "report_exported:EXPORT-ɛ",
        },
      }),
