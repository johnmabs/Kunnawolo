...
if (databaseUrl === undefined)
  throw new Error(
    "DATABASE_URL is required for estimated-result integration tests.",
  );
...
beforeAll(async () => {
  ...
  await prisma.saleCart.createMany({
    data: [
      {
        id: "estimated-result-sale",
        organizationId,
        shopId: inactiveShopId,
        status: "FINALIZED",
        finalizationReference: "RES-1",
        finalizedAt: at,
      },
      {
        id: "estimated-result-other-sale",
        organizationId: otherOrganizationId,
        shopId: otherShopId,
        status: "FINALIZED",
        finalizationReference: "RES-OTHER",
        finalizedAt: at,
      },
    ],
    skipDuplicates: true,
  ...
  await prisma.saleLine.createMany({
    data: [
      {
        id: "estimated-result-sale-line",
        saleCartId: "estimated-result-sale",
        productId,
        productNameSnapshot: "Nsiirin ɲa",
        quantity: 2,
        unitPriceMinor: 1000,
        unitCostMinor: 600,
        currency: "XOF",
      ...
      {
        id: "estimated-result-other-sale-line",
        saleCartId: "estimated-result-other-sale",
    ...
    data: [
      {
        ...
        currency: "XOF",
        reference: "RES-EXP",
        description: "Transport Ɛ",
        occurredAt: at,
      },
      {
        id: "estimated-result-cancelled-expense",
        organizationId,
        expenseCategoryId: categoryId,
        shopId: inactiveShopId,
        amountMinor: 500,
  ...
  await prisma.expenseCancellation.upsert({
    ...
    create: {
      id: "estimated-result-cancellation",
      organizationId,
      expenseId: "estimated-result-cancelled-expense",
      cancellationReference: "RES-ANN",
      reason: "Erreur",
      cancelledAt: at,
    },
    update: {},
  ...
  await prisma.stockLoss.createMany({
    data: [
      ...
      {
        id: "estimated-result-other-loss",
        organizationId: otherOrganizationId,
        shopId: otherShopId,
        productId: otherProductId,
        quantity: 1,
        reason: "Autre",
        referenceCostMinor: 9000,
        currency: "XOF",
        occurredAt: at,
      },
    ],
...
afterAll(async () => {
  ...
  await prisma.stockLoss.deleteMany({
    where: {
      id: { in: ["estimated-result-loss", "estimated-result-other-loss"] },
    },
  });
  await prisma.saleLine.deleteMany({
    where: {
      id: {
        in: ["estimated-result-sale-line", "estimated-result-other-sale-line"],
      },
    },
  });
  await prisma.saleCart.deleteMany({
    where: {
      id: { in: ["estimated-result-sale", "estimated-result-other-sale"] },
    },
  });
...
describe("estimated-result reporting", () => {
  it("uses immutable snapshots, excludes cancelled expenses, isolates organizations, and retains inactive-shop history", async () => {
    ...
    ).resolves.toMatchObject({ amount: { amountMinor: -1 } });
  });
});
