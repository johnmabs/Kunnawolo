  ...
  public async save(reportExport: ReportExport): Promise<void> {
    await this.prisma.$transaction(async (tx) => {
      await tx.reportExport.create({
        data: {
          id: reportExport.id.value,
          organizationId: reportExport.filter.organizationId.value,
          shopId: reportExport.filter.shopId?.value ?? null,
          format: "CSV",
          reference: reportExport.reference,
          actorId: reportExport.actorId.value,
          occurredFrom: reportExport.filter.occurredFrom,
          occurredTo: reportExport.filter.occurredTo,
      ...
      await tx.organizationAudit.create({
        data: {
          ...
          organizationId: reportExport.filter.organizationId.value,
          actorId: reportExport.actorId.value,
          action: `report_exported:${reportExport.reference}`,
        },
      });
    });
  }
