...
class Inventory implements InventoryReportingSource {
  public async projectStock() {
    return { onHandQuantity: 10, lossQuantity: 2, anomalyCount: 1 };
  }
}
class Transfers implements TransfersReportingSource {
  public async inTransitQuantity() {
    return 3;
  }
}
...
class Authorization implements ReportingReadAuthorization {
  public calls: Array<readonly [string, string | null, string | null]> = [];
  public async authorize(
    organizationId: string,
    shopId: string | null,
    actorId: string | null,
  ) {
    this.calls.push([organizationId, shopId, actorId]);
  }
}
class Exports implements ReportExportRepository {
  public values = new Map<
    string,
    import("../domain/report-export").ReportExport
  >();
  public async findByReference(organizationId: string, reference: string) {
    return this.values.get(`${organizationId}:${reference}`) ?? null;
  }
  public async save(
    reportExport: import("../domain/report-export").ReportExport,
  ) {
    this.values.set(
      `${reportExport.filter.organizationId.value}:${reportExport.reference}`,
      reportExport,
    );
  }
}
...
describe("ExportDashboardCsv", () => {
  it("authorizes and persists an idempotent, explainable CSV snapshot", async () => {
    ...
    const useCase = new ExportDashboardCsv(
      view,
      exports,
      { next: () => Identifier.fromString("export-id") },
