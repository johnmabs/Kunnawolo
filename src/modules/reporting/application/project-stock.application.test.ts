...
class InventorySource implements InventoryReportingSource {
  public async projectStock() {
    return { onHandQuantity: 10, lossQuantity: 2, anomalyCount: 1 };
  }
}
class TransfersSource implements TransfersReportingSource {
...
describe("ProjectStock", () => {
  it("combines explicit inventory and transfer sources without owning source facts", async () => {
    await expect(
      new ProjectStock(new InventorySource(), new TransfersSource()).execute({
        organizationId: "org",
        shopId: "inactive-shop",
      }),
    ).resolves.toMatchObject({
      shopId: { value: "inactive-shop" },
      onHandQuantity: { value: 10 },
