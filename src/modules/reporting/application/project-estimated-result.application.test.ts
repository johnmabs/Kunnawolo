...
describe("ProjectEstimatedResult", () => { it("combines explicit reporting sources", async () => { await expect(new ProjectEstimatedResult(new Sales(), new Expenses(), new Losses()).execute({ organizationId: "org", shopId: "inactive-shop" })).resolves.toMatchObject({ amount: { amountMinor: 500 }, activeExpenses: { amountMinor: 300 }, valuedLosses: { amountMinor: 200 } }); }); });
