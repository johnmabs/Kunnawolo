  ...
  public static create(
    ...
    if (!Number.isFinite(input.quantityDelta) || input.quantityDelta === 0)
      throw new DomainError(
        "inventory.invalid_movement_quantity",
        "A stock movement quantity must be non-zero.",
      );
    const reason = input.reason.trim().normalize("NFC");
    if (reason.length === 0)
      throw new DomainError(
        "inventory.invalid_movement_reason",
        "A movement reason must be non-empty.",
      );
    const idempotencyKey =
      input.idempotencyKey?.trim().normalize("NFC") || null;
