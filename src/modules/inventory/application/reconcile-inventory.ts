...
export class ReconcileInventory {
  public constructor(
    private readonly reconciliations: InventoryReconciliationRepository,
  ...
  public async execute(
    ...
    const snapshots = await this.reconciliations.findSnapshots(
      input.organizationId,
      input.shopId,
    );
    const divergences = snapshots.filter(
    ...
    const reconciliation = InventoryReconciliation.detect({
      ...
      snapshots,
      lineIds: divergences.map(() => this.ids.next()),
    });
