...
export class ApplyInventoryAdjustment {
  public constructor(
    ...
    private readonly adjustments: InventoryAdjustmentRepository,
  ...
  public async execute(
    ...
    const session = await this.sessions.findClosed(
    ...
    const adjustment = InventoryAdjustment.fromClosedSession({
      ...
      lineIds: discrepancies.map(() => this.ids.next()),
      movementIds: discrepancies.map(() => this.ids.next()),
    });
