...
export class ListStock {
  ...
  public async execute(input: Readonly<{ organizationId: string; shopId: string; productSearch?: string | null }>): Promise<StockListProjection> {
    ...
    const projection = await this.projections.listStock(organizationId, shopId, productSearch);
