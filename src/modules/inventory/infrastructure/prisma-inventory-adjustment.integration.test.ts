...
if (databaseUrl === undefined)
  throw new Error(
    "DATABASE_URL is required for inventory adjustment integration tests.",
  );
...
let sequence = 0;
const ids = {
  next: () => Identifier.fromString(`adjustment-id-${++sequence}`),
};
...
beforeAll(async () => {
  await prisma.organization.upsert({
    ...
    create: { id: organizationId, name: "Ajustements Ɛ", currency: "XOF" },
    update: {},
  });
...
afterAll(async () => {
  await prisma.inventoryAdjustmentLine.deleteMany({
    where: { inventoryAdjustment: { organizationId } },
  });
  ...
  await prisma.inventorySessionLine.deleteMany({
    where: { inventorySession: { organizationId } },
  });
...
describe("PrismaInventoryAdjustmentRepository", () => {
  it("applies a closed-session discrepancy once, atomically, after shop deactivation", async () => {
    const apply = new ApplyInventoryAdjustment(
      new PrismaInventorySessionRepository(prisma),
    ...
    const adjustment = await apply.execute({
      organizationId,
      sessionId,
      reference: "  AJU-Ɛ  ",
      actorId: "actor",
    ...
    await expect(
      ...
      lines: [
        {
          expectedQuantity: expect.anything(),
          countedQuantity: expect.anything(),
          quantityDelta: expect.anything(),
        },
      ],
    });
    await expect(
      prisma.stockMovement.count({
        where: { organizationId, reason: "inventory.adjustment:AJU-Ɛ" },
      }),
    ).resolves.toBe(1);
    await expect(
      prisma.organizationAudit.count({
        where: {
          organizationId,
          actorId: "actor",
          action: `inventory_adjustment.applied:${adjustment.id.value}:AJU-Ɛ`,
        },
      }),
    ).resolves.toBe(1);
    await expect(
    ...
    ).rejects.toMatchObject({ code: "inventory.closed_session_not_found" });
  });
});
