...
import type {
  InventoryReportingData,
  InventoryReportingSource,
} from "../application/ports/inventory-reporting-source";
...
export class PrismaInventoryReportingSource implements InventoryReportingSource {
  ...
  public async projectStock(
  ...
  ): Promise<InventoryReportingData> {
    ...
    const [levels, losses, anomalyCount] = await Promise.all([
      this.prisma.stockLevel.aggregate({
        where: { organizationId: input.organizationId, ...shopScope },
        _sum: { quantity: true },
      }),
      ...
      this.prisma.inventoryReconciliationLine.count({
        where: {
    ...
    return {
      onHandQuantity: Number(levels._sum.quantity ?? 0),
      lossQuantity: Number(losses._sum.quantity ?? 0),
      anomalyCount,
    };
  }
}
