...
export class PrismaInventoryProjectionRepository implements InventoryProjectionRepository {
  ...
  public async listStock(
          ...
          : {
              product: {
                OR: [
                  ...
                  { barcode: { contains: productSearch, mode: "insensitive" } },
                ],
              },
            }),
      },
    ...
    return StockListProjection.create({
      organizationId,
      shopId,
      shopName: shop.name,
      items: rows.map((row) => ({
  ...
  public async listSessions(
    ...
    return InventorySessionListProjection.create({
      ...
      shopName: shop.name,
      items: rows.map((row) => {
        const counted = row.lines.filter(
          (line) => line.countedQuantity !== null,
        );
        const discrepancies = counted
          .map(
            (line) =>
              Number(line.countedQuantity) - Number(line.expectedQuantity),
          )
        ...
        return {
          ...
          totalLineCount: row.lines.length,
          countedLineCount: counted.length,
          discrepancyLineCount: discrepancies.length,
          discrepancyQuantity: discrepancies.reduce(
            (total, difference) => total + Math.abs(difference),
            0,
          ),
        };
      }),
  ...
  public async listMovements(
    ...
    const rows = await this.prisma.stockMovement.findMany({
      where: { organizationId, shopId, productId },
      orderBy: [{ occurredAt: "desc" }, { id: "desc" }],
    });
    return StockMovementHistoryProjection.create({
      organizationId,
      shopId,
      productId,
  ...
  public async listLosses(
    ...
    return StockLossHistoryProjection.create({
      organizationId,
      shopId,
      productId,
      items: rows.map((row) => ({
        ...
        quantity: Number(row.quantity),
        reason: row.reason,
        referenceCostMinor: Number(row.referenceCostMinor),
