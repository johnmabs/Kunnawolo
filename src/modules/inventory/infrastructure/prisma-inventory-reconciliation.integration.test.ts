...
if (databaseUrl === undefined)
  throw new Error(
    "DATABASE_URL is required for inventory reconciliation integration tests.",
  );
...
let sequence = 0;
const ids = {
  next: () => Identifier.fromString(`reconciliation-id-${++sequence}`),
};
...
beforeAll(async () => {
  await prisma.organization.upsert({
    ...
    create: { id: organizationId, name: "Rapprochement ŋ", currency: "XOF" },
    update: {},
  });
  ...
  await prisma.stockLevel.upsert({
    where: {
      organizationId_shopId_productId: { organizationId, shopId, productId },
    },
    create: {
  ...
  await prisma.stockMovement.createMany({
    data: {
      id: "reconciliation-movement",
      organizationId,
      shopId,
      productId,
      quantityDelta: 5,
      reason: "stock.entry:RAPP-ɔ",
      actorId: "actor",
      idempotencyKey: "reconciliation-entry",
      occurredAt: now,
    },
    skipDuplicates: true,
  });
...
afterAll(async () => {
  await prisma.inventoryReconciliationLine.deleteMany({
    where: { inventoryReconciliation: { organizationId } },
  });
...
describe("PrismaInventoryReconciliationRepository", () => {
  it("persists a divergence once in an inactive historical shop and isolates organizations", async () => {
    const reconcile = new ReconcileInventory(
      new PrismaInventoryReconciliationRepository(prisma),
      ids,
      { now: () => now },
    );
    ...
    await expect(
      prisma.inventoryReconciliation.findFirst({
        where: { organizationId, reference: "RAPP-ɔ" },
        include: { lines: true },
      }),
      ...
      lines: [
        {
          productId,
          stockLevelQuantity: expect.anything(),
          ledgerQuantity: expect.anything(),
          quantityDifference: expect.anything(),
        },
      ],
    });
    ...
    await expect(
      prisma.inventoryReconciliation.count({ where: { organizationId } }),
    ).resolves.toBe(1);
    await expect(
      reconcile.execute({
