...
function toStockLevel(
  ...
  return StockLevel.reconstitute(
    ...
    Identifier.fromString(row.productId),
    Quantity.fromNumber(Number(row.quantity.toString())),
    Quantity.fromNumber(Number(row.lowStockThreshold.toString())),
  );
}
...
export class PrismaStockLevelRepository implements StockLevelRepository {
      ...
      const existing = await transaction.stockLevel.findUnique({
        where: {
          organizationId_shopId_productId: {
            organizationId: level.organizationId.value,
            shopId: level.shopId.value,
            productId: level.productId.value,
          },
        },
      });
      if (existing !== null) return toStockLevel(existing);
      const created = await transaction.stockLevel.create({
        data: {
          id: level.id.value,
          organizationId: level.organizationId.value,
          shopId: level.shopId.value,
  ...
  public async setLowStockThreshold(
    level: StockLevel,
    actorId: string | null,
  ): Promise<StockLevel> {
    const row = await this.prisma.$transaction(async (transaction) => {
      const updated = await transaction.stockLevel.update({
        where: { id: level.id.value },
        data: { lowStockThreshold: level.lowStockThreshold.value },
      });
      await transaction.organizationAudit.create({
        data: {
          id: crypto.randomUUID(),
          organizationId: level.organizationId.value,
          actorId,
          action: "stock_level.low_threshold_set",
        },
      });
      return updated;
    });
  ...
  public async findLowStock(
    ...
    const rows = await this.prisma.stockLevel.findMany({
      where: { organizationId, shopId, lowStockThreshold: { gt: 0 } },
    });
