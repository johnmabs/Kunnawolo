...
export class OperationalAlertPageQuery {
  private constructor(
    ...
    public readonly cursor: Readonly<{ occurredAt: Date; id: string }> | null,
  ) {}
  ...
  ): OperationalAlertPageQuery {
    const limit = input.limit ?? 25;
    if (!Number.isSafeInteger(limit) || limit < 1 || limit > 100)
      throw new DomainError(
        "observability.invalid_page_limit",
        "The alert page limit must be between 1 and 100.",
      );
    const cursor =
      input.cursor === null || input.cursor === undefined
        ? null
        : OperationalAlertPageQuery.parseCursor(input.cursor);
    return new OperationalAlertPageQuery(
  ...
  private static parseCursor(
    ...
    if (
      ...
      throw new DomainError(
        "observability.invalid_page_cursor",
        "The alert page cursor is invalid.",
      );
    return { occurredAt, id };
  }

  public static cursorOf(
    item: Readonly<{ occurredAt: Date; id: string }>,
  ): string {
    return `${item.occurredAt.toISOString()}|${item.id}`;
  }
}
