...
describe("OperationalAlertPageQuery", () => {
  it("bounds pagination and round-trips an opaque cursor", () => {
    const cursor = OperationalAlertPageQuery.cursorOf({
      id: "alert-ɛ",
    ...
    ).toMatchObject({ limit: 2, cursor: { id: "alert-ɛ" } });
    expect(() =>
      OperationalAlertPageQuery.create({ organizationId: "org", limit: 101 }),
    ).toThrow(
      expect.objectContaining({ code: "observability.invalid_page_limit" }),
    );
  });
});
