...
export type ObservationMetadata = Readonly<
...
export class OperationalObservation {
  ...
  public static create(
    ...
    if (
      ...
      throw new DomainError(
        "observability.invalid_observation",
        "An observation action, reference, and correlation id are required.",
      );
    if (!Number.isSafeInteger(input.durationMillis) || input.durationMillis < 0)
      throw new DomainError(
        "observability.invalid_duration",
        "An observation duration must be a non-negative integer.",
      );
  ...
  public isSlow(): boolean {
    return this.durationMillis >= 5000;
  }
}
