...
beforeAll(async () => {
  await prisma.organization.createMany({ data: [{ id: organizationId, name: "Observabilité Ɛ", currency: "XOF" }, { id: otherOrganizationId, name: "Autre", currency: "XOF" }], skipDuplicates: true });
  await prisma.shop.createMany({ data: [{ id: inactiveShopId, organizationId, code: "OBS-Ɛ", name: "Historique Ɔ" }, { id: otherShopId, organizationId: otherOrganizationId, code: "OBS-Ɛ", name: "Autre" }], skipDuplicates: true });
  await prisma.shop.update({ where: { id: inactiveShopId }, data: { isActive: false } });
});

afterAll(async () => {
  await prisma.operationalAlert.deleteMany({ where: { organizationId: { in: [organizationId, otherOrganizationId] } } });
  await prisma.operationalMetric.deleteMany({ where: { organizationId: { in: [organizationId, otherOrganizationId] } } });
...
describe("Prisma operational observability", () => {
  it("records an atomic, idempotent audit/metric/alert triplet for an inactive shop without crossing organizations", async () => {
    ...
    const observation = OperationalObservation.create({ organizationId, shopId: inactiveShopId, actorId: null, action: "report.exported", reference: "EXP-ɛ", correlationId: "observability-correlation", durationMillis: 5000, metadata: { format: "CSV", unicode: "ɛɔɲŋ" }, occurredAt: at });
    await repository.record(observation);
    await repository.record(observation);

    await expect(prisma.organizationAudit.findMany({ where: { organizationId, correlationId: "observability-correlation" } })).resolves.toMatchObject([{ shopId: inactiveShopId, reference: "EXP-ɛ", metadata: { unicode: "ɛɔɲŋ" } }]);
