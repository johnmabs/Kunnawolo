...
export class PrismaOperationalObservabilityRepository implements OperationalObservabilityRepository {
  ...
  public async record(observation: OperationalObservation): Promise<void> {
    await this.prisma.$transaction(async (tx) => {
      if (
        observation.shopId !== null &&
        (await tx.shop.count({
          where: {
            id: observation.shopId.value,
      ...
      const existing = await tx.organizationAudit.findFirst({
        where: {
          organizationId: observation.organizationId.value,
      ...
      await tx.organizationAudit.create({
        data: {
          ...
          actorId: observation.actorId?.value ?? null,
      ...
      await tx.operationalMetric.create({
        data: {
          id: crypto.randomUUID(),
          organizationId: observation.organizationId.value,
          shopId: observation.shopId?.value ?? null,
          name: "operation.duration_ms",
          value: BigInt(observation.durationMillis),
          correlationId: observation.correlationId,
          observedAt: observation.occurredAt,
        },
      });
      if (observation.isSlow())
