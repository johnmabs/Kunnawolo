...
class Repository implements OperationalObservabilityRepository {
  public observations: OperationalObservation[] = [];
  public async record(observation: OperationalObservation) {
    this.observations.push(observation);
  }
}
class Logger implements OperationalLogger {
  public events: unknown[] = [];
  public info(event: Parameters<OperationalLogger["info"]>[0]) {
...
describe("ObserveOperation", () => {
  it("records and logs an explainable operation without exposing extra data", async () => {
    ...
    await new ObserveOperation(repository, logger, {
