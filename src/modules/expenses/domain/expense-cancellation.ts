  ...
  public static create(
    ...
    if (reference.length === 0)
      throw new DomainError(
        "expenses.invalid_cancellation_reference",
        "An expense cancellation reference must be non-empty.",
      );
    if (reason.length === 0)
      throw new DomainError(
        "expenses.invalid_cancellation_reason",
        "An expense cancellation reason must be non-empty.",
      );
