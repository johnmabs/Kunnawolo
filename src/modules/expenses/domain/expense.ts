  ...
  public static record(
    ...
    if (!input.amount.isPositive())
      throw new DomainError(
        "expenses.invalid_amount",
        "An expense amount must be strictly positive.",
      );
    const reference = input.reference.trim().normalize("NFC");
    if (reference.length === 0)
      throw new DomainError(
        "expenses.invalid_reference",
        "An expense reference must be non-empty.",
      );
    const description = input.description.trim().normalize("NFC");
    if (description.length === 0)
      throw new DomainError(
        "expenses.invalid_description",
        "An expense description must be non-empty.",
      );
    return new Expense(
      input.id,
