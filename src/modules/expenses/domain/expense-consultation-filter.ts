...
export class ExpenseConsultationFilter {
  ...
  ): ExpenseConsultationFilter {
    const status = input.status ?? "ACTIVE";
    if (status !== "ALL" && status !== "ACTIVE" && status !== "CANCELLED")
      throw new DomainError(
        "expenses.invalid_status_filter",
        "The expense status filter is unsupported.",
      );
    if (
      ...
      throw new DomainError(
        "expenses.invalid_date_range",
        "The expense date range is invalid.",
      );
    const query = input.query?.trim().normalize("NFC") ?? null;
    return new ExpenseConsultationFilter(
      ...
      input.categoryId === null || input.categoryId === undefined
        ? null
        : Identifier.fromString(input.categoryId),
      query === "" ? null : query,
      input.occurredFrom ?? null,
      input.occurredTo ?? null,
      status,
