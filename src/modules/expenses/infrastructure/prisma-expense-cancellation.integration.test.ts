...
if (databaseUrl === undefined)
  throw new Error(
    "DATABASE_URL is required for expense cancellation integration tests.",
  );
...
let sequence = 0;
const ids = {
  next: () => Identifier.fromString(`expense-cancellation-${++sequence}`),
};
...
beforeAll(async () => {
  await prisma.organization.createMany({
    data: [
      { id: organizationId, name: "Annulations Ɛ", currency: "XOF" },
  ...
  await prisma.expenseCategory.createMany({
    data: [
      { id: categoryId, organizationId, name: "Transport", isActive: true },
    ],
    skipDuplicates: true,
  });
...
afterAll(async () => {
  await prisma.expenseCancellation.deleteMany({
...
describe("PrismaExpenseCancellationRepository", () => {
  it("cancels an immutable expense in an inactive historical shop exactly once", async () => {
    ...
    await expect(cancel.execute(input)).resolves.toMatchObject({
      expenseId: { value: expense.id.value },
      reference: "ANN-Ɛ-001",
      reason: "Saisie Ɛ erronée",
    });
    await expect(
      cancel.execute({ ...input, reason: "Autre" }),
    ).resolves.toMatchObject({
      reference: "ANN-Ɛ-001",
      reason: "Saisie Ɛ erronée",
    });
    await expect(
      cancel.execute({ ...input, reference: "ANN-Ɛ-002" }),
    ).rejects.toMatchObject({ code: "expenses.already_cancelled" });
    await expect(
      cancel.execute({
        ...input,
    ...
    ).rejects.toMatchObject({ code: "expenses.expense_not_found" });
    await expect(
      prisma.expense.findUnique({ where: { id: expense.id.value } }),
    ).resolves.toMatchObject({
      shopId,
      amountMinor: BigInt(3500),
      description: "Transport ɲa",
    });
    await expect(
      prisma.expenseCancellation.count({
        where: { organizationId, expenseId: expense.id.value },
      }),
    ).resolves.toBe(1);
    await expect(
      prisma.organizationAudit.count({
        where: { organizationId, action: "expense.cancelled:ANN-Ɛ-001" },
      }),
    ).resolves.toBe(1);
  });
});
