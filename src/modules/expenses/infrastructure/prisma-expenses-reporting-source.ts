...
import type { ExpensesReportingSource } from "../application/ports/expenses-reporting-source";
export class PrismaExpensesReportingSource implements ExpensesReportingSource {
  ...
  public async activeExpenseAmount(
    ...
    for (const row of rows) {
      if (row.currency !== org.currency)
        throw new DomainError(
          "reporting.invalid_expense_snapshot",
          "An expense currency does not match its organization.",
        );
