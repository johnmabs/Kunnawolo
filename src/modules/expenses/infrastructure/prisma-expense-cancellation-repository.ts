...
export class PrismaExpenseCancellationRepository implements ExpenseCancellationRepository {
  ...
  public async findByReference(
    ...
    const row = await this.prisma.expenseCancellation.findFirst({
  ...
  public async cancel(
    ...
    return this.prisma.$transaction(async (tx) => {
      const byReference = await tx.expenseCancellation.findFirst({
        where: {
      ...
      if (byReference !== null) {
        if (byReference.expenseId === cancellation.expenseId.value)
          return toCancellation(byReference);
        throw new DomainError(
          "expenses.cancellation_reference_taken",
          "The expense cancellation reference is already used.",
        );
      }
      const expense = await tx.expense.findFirst({
        where: {
          id: cancellation.expenseId.value,
          organizationId: cancellation.organizationId.value,
        },
      });
      if (expense === null)
        throw new DomainError(
          ...
          "The expense does not belong to this organization.",
        );
      const existingCancellation = await tx.expenseCancellation.findUnique({
        where: { expenseId: cancellation.expenseId.value },
      });
      if (existingCancellation !== null)
        throw new DomainError(
          "expenses.already_cancelled",
          "The expense has already been cancelled.",
        );
      await tx.expenseCancellation.create({
        data: {
          id: cancellation.id.value,
          organizationId: cancellation.organizationId.value,
          expenseId: cancellation.expenseId.value,
          cancellationReference: cancellation.reference,
      ...
      await tx.organizationAudit.create({
        data: {
          ...
          organizationId: cancellation.organizationId.value,
          actorId: cancellation.actorId,
          action: `expense.cancelled:${cancellation.reference}`,
        },
      });
      return cancellation;
    });
