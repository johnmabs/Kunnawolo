...
if (databaseUrl === undefined)
  throw new Error(
    "DATABASE_URL is required for expense category integration tests.",
  );
...
describe("PrismaExpenseCategoryRepository", () => {
  it("persists a unique Unicode category within its organization", async () => {
    const repository = new PrismaExpenseCategoryRepository(prisma);
    const create = new CreateExpenseCategory(repository, {
      next: () =>
        Identifier.fromString(
          sequence++ === 0 ? categoryId : `expense-category-${sequence}`,
        ),
    });
    ...
    await expect(
      new UpdateExpenseCategory(repository).execute({
        organizationId,
        categoryId,
        name: "Transport Ɛ",
        isActive: false,
    ...
    await expect(
      create.execute({ organizationId, name: "Transport Ɛ", actorId: "actor" }),
    ).rejects.toMatchObject({ code: "expenses.duplicate_category_name" });
    await expect(
      repository.findById(otherOrganizationId, categoryId),
    ).resolves.toBeNull();
    await expect(
      prisma.organizationAudit.count({ where: { organizationId } }),
    ).resolves.toBe(2);
  });
});
