...
import type {
  ExpenseConsultationRepository,
  ExpenseListItem,
} from "../application/ports/expense-consultation-repository";
...
function toItem(row: ExpenseRow): ExpenseListItem {
  ...
  const cancellation =
    row.cancellation === null
      ? null
      : ExpenseCancellation.create({
          id: Identifier.fromString(row.cancellation.id),
          organizationId: Identifier.fromString(
            row.cancellation.organizationId,
          ),
          expenseId: Identifier.fromString(row.cancellation.expenseId),
          reference: row.cancellation.cancellationReference,
          reason: row.cancellation.reason,
          actorId: row.cancellation.actorId,
          cancelledAt: row.cancellation.cancelledAt,
        });
    ...
    const rows = await this.prisma.expense.findMany({
      where: {
        ...
        ...(filter.categoryId === null
          ? {}
          : { expenseCategoryId: filter.categoryId.value }),
        ...(filter.query === null
          ? {}
          : {
              OR: [
                { reference: { contains: filter.query, mode: "insensitive" } },
                {
                  description: { contains: filter.query, mode: "insensitive" },
                },
              ],
            }),
        ...(filter.occurredFrom === null && filter.occurredTo === null
          ? {}
          : {
              occurredAt: {
