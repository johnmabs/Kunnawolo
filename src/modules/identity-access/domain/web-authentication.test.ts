...
describe("web authentication domain", () => {
  it("accepts long unicode passphrases without normalization", () => {
    const password = " phrase secrète 🔐 très longue ";
    expect(PasswordPolicy.validate(password)).toBe(password);
    expect(() => PasswordPolicy.validate("trop court")).toThrow(
      expect.objectContaining({ code: "auth.password_too_short" }),
    );
  });
  ...
  it("accepts an invitation only once before expiry", () => {
    const now = new Date("2026-09-02T10:00:00Z");
    const invitation = MembershipInvitation.issue({
      id: id("invitation"),
      organizationId: id("org"),
      membershipId: id("member"),
      invitedByActorId: id("owner"),
    ...
    expect(() => invitation.accept(now).accept(now)).toThrow(
      expect.objectContaining({ code: "auth.invitation_already_accepted" }),
    );
  });
});
