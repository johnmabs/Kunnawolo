...
export async function queryWorkspaceSession(prisma: PrismaClient, account: UserAccount, preferredOrganizationId: string | null = null) {
  ...
  return {
    ...
    currentOrganizationId: organizations.some(({ id }) => id === preferredOrganizationId) ? preferredOrganizationId! : organizations[0]?.id ?? "",
    organizations,
  };
}
