...
import type { ApiSecretHash, ApiSecretHasher } from "../application/ports/api-secret-hasher";

const scryptAsync = promisify(scrypt);

export class NodeApiSecretGenerator implements ApiSecretGenerator {
  public generate(): string { return randomBytes(32).toString("base64url"); }
}
...
export class NodeApiSecretHasher implements ApiSecretHasher {
  public async create(secret: string): Promise<ApiSecretHash> {
    ...
    return { salt, hash: (await scryptAsync(secret, salt, 32) as Buffer).toString("base64url") };
  }
