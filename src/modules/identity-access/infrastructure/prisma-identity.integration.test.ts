...
beforeAll(async () => {
  await prisma.organization.upsert({
    ...
    create: { id: organizationId, name: "Identity", currency: "XOF" },
    update: {},
  });
  await prisma.userAccount.upsert({
    where: { id: accountId },
    create: { id: accountId, email: "test@example.com", displayName: "Tɛst" },
    update: {},
  });
});
afterAll(async () => {
...
describe("OrganizationMembership persistence", () => {
  it("keeps membership separate from the user account", async () => {
    await prisma.organizationMembership.create({
      data: {
        id: "identity-integration-membership",
        organizationId,
    ...
    ).resolves.toMatchObject({ status: "INVITED", userAccountId: accountId });
  });
});
