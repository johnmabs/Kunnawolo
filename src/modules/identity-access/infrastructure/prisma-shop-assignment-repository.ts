...
export class PrismaShopAssignmentRepository implements ShopAssignmentRepository {
  ...
  public async assign(
    input: Readonly<{ id: string; membershipId: string; shopId: string }>,
  ): Promise<void> {
    await this.prisma.shopAssignment.upsert({
      where: {
        membershipId_shopId: {
  ...
  public async findShopIdsForMembership(
    membershipId: string,
  ): Promise<readonly string[]> {
    return (
      await this.prisma.shopAssignment.findMany({
        where: { membershipId },
        select: { shopId: true },
      })
