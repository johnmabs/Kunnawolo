...
import type {
  ...
  ApiKeyAudit,
} from "../application/ports/api-access-key-repository";
...
export class PrismaApiAccessKeyRepository implements ApiAccessKeyRepository {
  ...
  public async findByOrganizationAndId(
    organizationId: string,
    id: string,
  ): Promise<ApiAccessKey | null> {
    const row = await this.prisma.apiAccessKey.findFirst({
  ...
  public async save(key: ApiAccessKey, audit: ApiKeyAudit): Promise<void> {
    await this.prisma.$transaction(async (tx) => {
      const duplicate = await tx.apiAccessKey.findFirst({
        where: {
          organizationId: key.organizationId.value,
          label: key.label,
          NOT: { id: key.id.value },
        },
      });
      if (duplicate !== null)
        throw new DomainError(
          "security.api_key_label_taken",
          "An API key label must be unique in its organization.",
        );
      await tx.apiAccessKey.upsert({
        where: { id: key.id.value },
        create: {
          id: key.id.value,
          organizationId: key.organizationId.value,
          actorId: key.actorId.value,
          label: key.label,
          secretSalt: key.secretSalt,
          secretHash: key.secretHash,
          expiresAt: key.expiresAt,
          revokedAt: key.revokedAt,
          createdAt: key.createdAt,
        },
        update: { revokedAt: key.revokedAt },
      });
