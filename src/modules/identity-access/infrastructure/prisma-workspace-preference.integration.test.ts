...
if (databaseUrl === undefined)
  throw new Error(
    "DATABASE_URL is required for workspace-preference integration tests.",
  );
...
beforeAll(async () => {
  await prisma.organization.createMany({
    data: [
      { id: organizationId, name: "Poste Ɛ", currency: "XOF" },
  ...
  await prisma.organizationMembership.createMany({
    data: [
      ...
      {
        id: "workspace-manager-membership",
        organizationId,
  ...
  await prisma.shopAssignment.upsert({
    where: {
      membershipId_shopId: {
        membershipId: "workspace-manager-membership",
        shopId: inactiveShopId,
      },
    },
    create: {
      id: "workspace-manager-assignment",
      membershipId: "workspace-manager-membership",
      shopId: inactiveShopId,
    },
    update: {},
...
afterAll(async () => {
  await prisma.workspaceIdempotencyRecord.deleteMany({
  ...
  await prisma.shopAssignment.deleteMany({
    where: { id: "workspace-manager-assignment" },
  });
  await prisma.organizationMembership.deleteMany({
    where: {
      id: {
        in: ["workspace-owner-membership", "workspace-manager-membership"],
      },
    },
  });
  ...
  it("persists an idempotent Unicode historical-shop preference within the authorized organization", async () => {
    ...
    await expect(
      prisma.workspacePreference.count({
        where: { organizationId, actorId: managerId },
      }),
    ).resolves.toBe(1);
    await expect(
      prisma.organizationAudit.count({
        where: { organizationId, action: "workspace.preference_saved" },
      }),
    ).resolves.toBe(1);
    await expect(
      save.execute({ organizationId, actorId: managerId, shopId: otherShopId }),
    ).rejects.toMatchObject({ code: "workspace.shop_not_found" });
    await expect(
      save.execute({ organizationId, actorId: managerId, shopId: null }),
    ).rejects.toMatchObject({ code: "workspace.preference_forbidden" });
    const resilientSave = new SaveWorkspacePreferenceIdempotently(
      preferences,
      preferences,
      authorization,
    );
    const first = await resilientSave.execute({
      organizationId,
      actorId: managerId,
      shopId: inactiveShopId,
      isCompact: false,
      idempotencyKey: "poste-ɛ",
    });
    await expect(
      resilientSave.execute({
        organizationId,
        actorId: managerId,
        shopId: inactiveShopId,
        isCompact: false,
    ...
    ).rejects.toMatchObject({ code: "workspace.idempotency_conflict" });
    expect(first.isCompact).toBe(false);
    await expect(
      prisma.workspaceIdempotencyRecord.count({
        where: { organizationId, actorId: managerId },
      }),
    ).resolves.toBe(1);
  });
