...
export class PrismaWorkspacePreferenceRepository
  implements WorkspacePreferenceRepository, WorkspaceIdempotencyRepository
{
  ...
  public async find(
    ...
    const row = await this.prisma.workspacePreference.findUnique({
      where: { organizationId_actorId: { organizationId, actorId } },
    });
  ...
  public async save(preference: WorkspacePreference): Promise<void> {
    await this.prisma.$transaction(async (tx) => {
      const existing = await tx.workspacePreference.findUnique({
        where: {
          organizationId_actorId: {
            organizationId: preference.organizationId.value,
            actorId: preference.actorId.value,
          },
        },
      });
      if (
        existing !== null &&
        existing.shopId === (preference.shopId?.value ?? null) &&
        existing.isCompact === preference.isCompact
      )
        return;
      ...
      await tx.organizationAudit.create({
        data: {
          ...
          actorId: preference.actorId.value,
          shopId: preference.shopId?.value ?? null,
          action: "workspace.preference_saved",
          reference: preference.id.value,
        },
      });
    });
  }
  ...
  public async saveIdempotently(
    ...
    return this.prisma.$transaction(async (tx) => {
      const replay = await tx.workspaceIdempotencyRecord.findUnique({
        where: {
      ...
      if (replay !== null) {
        if (replay.fingerprint !== fingerprint)
          throw new DomainError(
            ...
            "The idempotency key was already used for another preference.",
          );
        return WorkspacePreference.configure({
          id: preference.id.value,
          organizationId: replay.organizationId,
          actorId: replay.actorId,
          shopId: replay.shopId,
          isCompact: replay.isCompact,
        });
      }
      const existing = await tx.workspacePreference.findUnique({
        where: {
      ...
      await tx.workspaceIdempotencyRecord.create({
        data: {
          id: crypto.randomUUID(),
          organizationId: preference.organizationId.value,
          actorId: preference.actorId.value,
