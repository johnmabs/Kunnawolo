...
if (databaseUrl === undefined)
  throw new Error("DATABASE_URL is required for invitation integration tests.");
...
function aggregate(label: string, deliveryId: string) {
  const account = UserAccount.create(
    Identifier.fromString(`account-${label}-${suffix}`),
    `${label}-${suffix}@example.test`,
    `Compte ${label}`,
  );
  const membership = OrganizationMembership.invite(
    Identifier.fromString(`membership-${label}-${suffix}`),
    Identifier.fromString(organizationId),
    account.id,
  );
  const invitation = MembershipInvitation.issue({
    id: Identifier.fromString(`invitation-${label}-${suffix}`),
    organizationId: Identifier.fromString(organizationId),
    membershipId: membership.id,
    invitedByActorId: Identifier.fromString(ownerId),
    email: account.email,
    tokenHash: tokenHash(label),
    issuedAt: new Date("2026-09-03T10:00:00.000Z"),
  ...
  return {
    ...
    delivery: {
      ...
      organizationName: "Outbox",
      acceptanceUrl: `https://example.test/invitations/${label}`,
...
describe("PrismaMembershipInvitationRepository outbox", () => {
  beforeAll(async () => {
    ...
    await prisma.organization.create({
      data: { id: organizationId, name: "Outbox", currency: "XOF" },
    });
  ...
  it("commits the invitation and delivery atomically", async () => {
    ...
    await expect(
      prisma.invitationDeliveryOutbox.findUnique({ where: { id: deliveryId } }),
    ).resolves.toMatchObject({
      invitationId: first.invitation.id.value,
      status: "PENDING",
      attemptCount: 0,
    });
    ...
    await expect(
      repository.create({ ...second, createAccount: true }),
    ).rejects.toBeDefined();
    await expect(
      prisma.userAccount.findUnique({ where: { id: second.account.id.value } }),
    ).resolves.toBeNull();
    await expect(
      prisma.organizationMembership.findUnique({
        where: { id: second.membership.id.value },
      }),
    ).resolves.toBeNull();
    await expect(
      prisma.membershipInvitation.findUnique({
        where: { id: second.invitation.id.value },
      }),
    ).resolves.toBeNull();
  });
  ...
  it("invalidates the previous token and pending delivery when reissued", async () => {
    ...
    const invitation = MembershipInvitation.issue({
      id: initial.invitation.id,
      organizationId: initial.invitation.organizationId,
      membershipId: initial.invitation.membershipId,
      invitedByActorId: initial.invitation.invitedByActorId,
      email: initial.invitation.email,
      tokenHash: replacementHash,
      issuedAt: new Date("2026-09-04T10:00:00.000Z"),
      expiresAt,
    });
    const delivery = {
      ...initial.delivery,
      id: `delivery-resend-new-${suffix}`,
      acceptanceUrl: "https://example.test/invitations/replacement",
      expiresAt,
    };
    ...
    ).resolves.toMatchObject({ tokenHash: replacementHash, expiresAt });
    await expect(
      prisma.invitationDeliveryOutbox.findUnique({
        where: { id: initial.delivery.id },
      }),
    ).resolves.toMatchObject({ status: "CANCELLED", acceptanceUrl: null });
    await expect(
      prisma.invitationDeliveryOutbox.findUnique({
      ...
      status: "PENDING",
      acceptanceUrl: delivery.acceptanceUrl,
    });
  });
});
