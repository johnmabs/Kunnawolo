...
function aggregate(label: string, deliveryId: string) {
  ...
  const invitation = MembershipInvitation.issue({ id: Identifier.fromString(`invitation-${label}-${suffix}`), organizationId: Identifier.fromString(organizationId), membershipId: membership.id, invitedByActorId: Identifier.fromString(ownerId), email: account.email, tokenHash: (label === "first" ? "a" : "b").repeat(64), issuedAt: new Date("2026-09-03T10:00:00.000Z"), expiresAt: new Date("2026-09-05T10:00:00.000Z") });
