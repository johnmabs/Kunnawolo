...
export type ApiAccessContext = Readonly<{
...
export class AuthenticateApiKey {
  ...
  public async execute(tokenValue: string): Promise<ApiAccessContext> {
    ...
    if (
      key === null ||
      !key.isUsable(this.clock.now()) ||
      !(await this.hasher.verify(token.secret, {
        salt: key.secretSalt,
        hash: key.secretHash,
      }))
    )
    ...
    await this.authorization.authorizeActiveMembership(
