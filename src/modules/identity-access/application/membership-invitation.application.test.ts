...
} from "./ports/membership-invitation-repository";
...
class Invitations implements MembershipInvitationRepository {
  ...
  public accepted:
    Parameters<MembershipInvitationRepository["accept"]>[0] | null = null;
  public reissued:
    Parameters<MembershipInvitationRepository["reissue"]>[0] | null = null;
  ...
  public async create(
    ...
    this.details = {
      ...
      hasCredential: !input.createAccount,
      invitation: input.invitation,
      membership: input.membership,
      organizationName: "ASTU SARL",
    };
  }
  ...
  public async reissue(
    ...
    this.reissued = input;
    if (this.details)
      this.details = { ...this.details, invitation: input.invitation };
  }
...
describe("membership invitations", () => {
  ...
  it("creates a 48-hour one-use invitation for a new account", async () => {
    ...
    const issued = await new InviteMember(
      ...
      organizationId: "org",
      invitedByActorId: "owner",
      email: " User@example.com ",
      displayName: "User",
      organizationName: "ASTU SARL",
    });
    expect(issued.acceptanceUrl).toContain("/invitations/temporary-token");
    expect(issued.invitation.expiresAt.toISOString()).toBe(
  ...
  it("sets a password and activates a new invited account", async () => {
    ...
    const result = await new AcceptMembershipInvitation(
      ...
      authenticatedUserAccountId: null,
    });
    expect(result.shouldIssueSession).toBe(true);
    expect(repository.accepted).toMatchObject({
      membership: { status: "ACTIVE" },
      credential: { algorithm: "test" },
    });
  });

  it("requires the invited identity to be signed in for an existing account", async () => {
    const repository = new Invitations();
    repository.account = UserAccount.create(
      ids.next(),
      "user@example.com",
      "User",
    );
    ...
    await expect(
      new AcceptMembershipInvitation(
        repository,
        opaque,
        new Passwords(),
        clock,
        ...
        token: "temporary-token",
        password: null,
        authenticatedUserAccountId: "someone-else",
      }),
    ).rejects.toMatchObject({ code: "auth.invitation_login_required" });
  });
  ...
  it("rotates the token and queues a fresh delivery when sending again", async () => {
    ...
    const result = await new ResendMembershipInvitation(
      repository,
      ids,
      ...
      organizationId: "org",
      invitationId: repository.details!.invitation.id.value,
      actorId: "owner",
    });
