...
export class InviteMember {
  ...
  public async execute(input: Readonly<{ organizationId: string; invitedByActorId: string; email: string; displayName: string; organizationName: string }>) {
    ...
    await this.repository.create({ account, createAccount: existing === null, invitation, membership, delivery: { id: deliveryId, invitationId: invitation.id.value, email: account.email, displayName: account.displayName, organizationName: input.organizationName, acceptanceUrl, expiresAt } });
    return { membership, invitation, acceptanceUrl, deliveryId };
  }
}
