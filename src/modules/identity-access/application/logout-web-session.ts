...
export class LogoutWebSession {
  ...
  public async execute(token: string): Promise<void> {
    ...
    const session = await this.repository.findSessionByTokenHash(this.tokenHasher.hash(token));
    if (session !== null) await this.repository.saveSession(session.revoke(this.clock.now()));
  }
}
