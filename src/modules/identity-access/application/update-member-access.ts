...
export class UpdateMemberAccess {
  ...
  public async execute(
    ...
    const actor = await this.repository.findMember(
    ...
    if (target === null)
      throw new DomainError(
        "iam.membership_not_found",
        "The membership does not exist in this organization.",
      );
    if (!roles.includes(input.role as OrganizationRole))
      throw new DomainError(
        "iam.invalid_role",
        "The organization role is invalid.",
      );
    const role = input.role as OrganizationRole;
    const shopIds =
      role === "OWNER"
        ? []
        : [...new Set(input.shopIds.map((id) => id.trim()).filter(Boolean))];
    const activeShopIds = new Set(
      await this.repository.findActiveShopIds(input.organizationId),
    );
    if (shopIds.some((id) => !activeShopIds.has(id)))
      throw new DomainError(
        "iam.invalid_shop_assignment",
        "Every assigned shop must be active in this organization.",
      );
    if (target.status === "ACTIVE" && role !== "OWNER" && shopIds.length === 0)
      throw new DomainError(
        "iam.shop_assignment_required",
        "An active manager or cashier must be assigned to at least one shop.",
      );
    const member = { ...target, role, shopIds };
    await this.repository.updateAtomically({
