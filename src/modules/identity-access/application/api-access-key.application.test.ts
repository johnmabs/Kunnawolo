...
import type {
  ...
  ApiKeyAudit,
} from "./ports/api-access-key-repository";
...
class Keys implements ApiAccessKeyRepository {
  ...
  public async findById(id: string) {
    return this.key?.id.value === id ? this.key : null;
  }
  public async findByOrganizationAndId(organizationId: string, id: string) {
    return this.key?.organizationId.value === organizationId &&
      this.key.id.value === id
      ? this.key
      : null;
  }
  public async save(key: ApiAccessKey, audit: ApiKeyAudit) {
    this.key = key;
    this.audits.push(audit);
  }
}
class Authorization implements ApiKeyAccessAuthorization {
  public async authorizeActiveMembership() {}
  public async authorizeOwner() {}
}
class Secrets implements ApiSecretGenerator, ApiSecretHasher {
  public generate() {
    return "a".repeat(43);
  }
  public async create(secret: string): Promise<ApiSecretHash> {
    return { salt: "salt", hash: `hash:${secret}` };
  }
  public async verify(secret: string, stored: ApiSecretHash) {
    return stored.hash === `hash:${secret}`;
  }
}
...
describe("organization API keys", () => {
  it("issues, authenticates, and revokes an organization-scoped secret idempotently", async () => {
    ...
    const issue = new IssueOrganizationApiKey(
      keys,
      new Authorization(),
      { next: () => Identifier.fromString("key-id") },
      new Secrets(),
      new Secrets(),
      clock,
    );
    ...
    ).resolves.toEqual({ organizationId: "org", actorId: "owner" });
    await new RevokeOrganizationApiKey(
      keys,
      new Authorization(),
      clock,
    ).execute({ organizationId: "org", actorId: "owner", keyId: "key-id" });
    await expect(
      new AuthenticateApiKey(
        keys,
        new Authorization(),
    ...
    expect(keys.audits.map(({ action }) => action)).toEqual([
      "api_key.issued:Clé Ɛ",
      "api_key.revoked:Clé Ɛ",
    ]);
  });
});
