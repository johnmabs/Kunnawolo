...
export class AcceptMembershipInvitation {
  ...
  public async execute(
    input: Readonly<{
      ...
      authenticatedUserAccountId: string | null;
    }>,
  ) {
    ...
    if (details.hasCredential) {
      if (input.authenticatedUserAccountId !== details.account.id.value)
        throw new DomainError(
          "auth.invitation_login_required",
          "Sign in with the invited account before accepting.",
        );
    } else {
      if (input.password === null)
        throw new DomainError(
          "auth.invitation_password_required",
    ...
    return {
      ...
      shouldIssueSession: !details.hasCredential,
    };
  }
}
