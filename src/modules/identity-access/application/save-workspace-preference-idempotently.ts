...
export class SaveWorkspacePreferenceIdempotently {
  public constructor(
    ...
    private readonly idempotency: WorkspaceIdempotencyRepository,
  ...
  public async execute(
    ...
    const result = await this.idempotency.saveIdempotently(
      preference,
      key.value,
      fingerprint,
    );
    if (
      result.organizationId.value !== input.organizationId ||
      result.actorId.value !== input.actorId
    )
      throw new DomainError(
        "workspace.idempotency_conflict",
        "The idempotent result does not belong to this workspace.",
      );
    return result;
  }
}
