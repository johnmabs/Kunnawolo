...
if (databaseUrl === undefined)
  throw new Error(
    "DATABASE_URL is required for operational-release integration tests.",
  );
const prisma = createPrismaClient(databaseUrl);

afterAll(async () => {
  await prisma.operationalRelease.deleteMany({
    where: { reference: "REL-INT-ɛ" },
  });
...
describe("Prisma operational releases", () => {
  it("persists a Unicode release reference idempotently without tenant data", async () => {
    ...
    const release = new RegisterOperationalRelease(
      repository,
      { next: () => Identifier.fromString("release-integration") },
    ...
    await expect(
      release.execute({
        version: "1.0.0",
        reference: "REL-INT-ɛ",
        artifactSha: "c".repeat(64),
        actorId: "operator",
      }),
    ).resolves.toMatchObject({ reference: "REL-INT-ɛ" });
    await expect(
      release.execute({
        version: "1.0.0",
        reference: "REL-INT-ɛ",
    ...
    ).resolves.toMatchObject({ id: { value: "release-integration" } });
  });
});
