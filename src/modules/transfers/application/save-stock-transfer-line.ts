...
export class SaveStockTransferLine {
  ...
  public async execute(input: Readonly<{ organizationId: string; transferId: string; productId: string; quantity: number; actorId: string | null }>): Promise<StockTransferLine> {
    ...
    if (!(await this.scope.activeTrackedProductBelongsToOrganization(input.organizationId, input.productId))) {
      throw new DomainError("transfers.product_not_found", "The tracked product does not belong to this organization.");
    }
    const existing = transfer.lines.find(({ productId }) => productId.value === input.productId);
    ...
    await this.transfers.saveLine(input.organizationId, input.transferId, line, { organizationId: input.organizationId, actorId: input.actorId, action: `transfer.line_saved:${input.transferId}` });
    return line;
  }
}
