...
export class CreateStockTransfer {
  ...
  public async execute(input: Readonly<{ organizationId: string; sourceShopId: string; destinationShopId: string; actorId: string | null }>): Promise<StockTransfer> {
    ...
    const [sourceExists, destinationExists] = await Promise.all([
      this.scope.activeShopBelongsToOrganization(input.organizationId, input.sourceShopId),
      this.scope.activeShopBelongsToOrganization(input.organizationId, input.destinationShopId),
    ]);
    if (!sourceExists || !destinationExists) {
      throw new DomainError("transfers.shop_not_found", "Both transfer shops must be active in this organization.");
    }
    ...
    await this.transfers.create(transfer, { organizationId: input.organizationId, actorId: input.actorId, action: `transfer.created:${transfer.id.value}` });
    return transfer;
  }
}
