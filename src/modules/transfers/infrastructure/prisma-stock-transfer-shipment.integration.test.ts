...
beforeAll(async () => {
  await prisma.organization.upsert({ where: { id: organizationId }, create: { id: organizationId, name: "Expédition Ɛ", currency: "XOF" }, update: {} });
  await prisma.userAccount.createMany({ data: [{ id: managerId, email: "shipment-manager@example.test", displayName: "Manager" }, { id: cashierId, email: "shipment-cashier@example.test", displayName: "Cashier" }], skipDuplicates: true });
  await prisma.shop.upsert({ where: { id: sourceShopId }, create: { id: sourceShopId, organizationId, code: "SHP", name: "Source" }, update: { isActive: true } });
  await prisma.shop.upsert({ where: { id: destinationShopId }, create: { id: destinationShopId, organizationId, code: "DST", name: "Destination" }, update: { isActive: true } });
...
afterAll(async () => {
  ...
  await prisma.userAccount.deleteMany({ where: { id: { in: [managerId, cashierId] } } });
...
describe("PrismaStockTransferRepository shipment", () => {
  it("decrements source stock atomically and is idempotent after shop deactivation", async () => {
    ...
    const transfer = await new CreateStockTransfer(scope, repository, ids).execute({ organizationId, sourceShopId, destinationShopId, actorId: managerId });
    await new SaveStockTransferLine(scope, repository, ids).execute({ organizationId, transferId: transfer.id.value, productId, quantity: 2, actorId: managerId });
    const send = new SendStockTransfer(repository, new PrismaTransfersDispatchAuthorization(prisma), { now: () => now });
    await expect(send.execute({ organizationId, transferId: transfer.id.value, reference: "EXP-Ɛ", actorId: cashierId })).rejects.toMatchObject({ code: "transfers.dispatch_forbidden" });
    await prisma.shop.update({ where: { id: sourceShopId }, data: { isActive: false } });
    const shipment = await send.execute({ organizationId, transferId: transfer.id.value, reference: "  EXP-Ɛ  ", actorId: managerId });
    await expect(send.execute({ organizationId, transferId: transfer.id.value, reference: "EXP-Ɛ", actorId: managerId })).resolves.toMatchObject({ reference: shipment.reference });
    ...
    await prisma.shop.update({ where: { id: sourceShopId }, data: { isActive: true } });
    const draft = await new CreateStockTransfer(scope, repository, ids).execute({ organizationId, sourceShopId, destinationShopId, actorId: managerId });
    const cancellation = new CancelStockTransfer(repository, new PrismaTransfersDispatchAuthorization(prisma), { now: () => now });
    await expect(cancellation.execute({ organizationId, transferId: draft.id.value, reference: "ANN-Ɛ", reason: "  Écart ɲa  ", actorId: cashierId })).rejects.toMatchObject({ code: "transfers.dispatch_forbidden" });
    await cancellation.execute({ organizationId, transferId: draft.id.value, reference: "ANN-Ɛ", reason: "  Écart ɲa  ", actorId: managerId });
    await cancellation.execute({ organizationId, transferId: draft.id.value, reference: "ANN-Ɛ", reason: "Écart ɲa", actorId: managerId });
    await expect(prisma.stockTransfer.findUnique({ where: { id: draft.id.value } })).resolves.toMatchObject({ status: "CANCELLED", cancellationReason: "Écart ɲa" });
  });
});
