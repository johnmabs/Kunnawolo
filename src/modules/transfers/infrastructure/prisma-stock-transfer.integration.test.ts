...
if (databaseUrl === undefined)
  throw new Error(
    "DATABASE_URL is required for stock transfer integration tests.",
  );
...
beforeAll(async () => {
  await prisma.organization.createMany({
    data: [
      { id: organizationId, name: "Transferts Ɛ", currency: "XOF" },
  ...
  await prisma.shop.upsert({
    ...
    create: { id: sourceShopId, organizationId, code: "SRC", name: "Source Ɛ" },
...
describe("PrismaStockTransferRepository", () => {
  it("persists an organization-scoped draft and valid Unicode line", async () => {
    ...
    await expect(
      ...
      destinationShopId,
      status: "DRAFT",
      lines: [{ quantity: expect.anything() }],
    });
    expect(
      Number(
        (
          await prisma.stockTransferLine.findFirstOrThrow({
            where: { stockTransferId: transfer.id.value },
          })
        ).quantity,
      ),
    ).toBe(3);
    await expect(
      repository.findDraft(otherOrganizationId, transfer.id.value),
    ).resolves.toBeNull();
    await expect(
      prisma.organizationAudit.count({
        where: { organizationId, action: { startsWith: "transfer." } },
      }),
    ).resolves.toBe(3);
    ...
    await expect(
      repository.findDraft(organizationId, transfer.id.value),
    ).resolves.toMatchObject({
      sourceShopId: { value: sourceShopId },
      lines: [{ quantity: { value: 3 } }],
    });
  });
  ...
  it("rejects shops from another organization and untracked products", async () => {
    ...
    await expect(
      create.execute({
        organizationId,
        sourceShopId,
        destinationShopId: "missing",
        actorId: null,
      }),
    ).rejects.toMatchObject({ code: "transfers.shop_not_found" });
    await expect(
      create.execute({
        organizationId,
        sourceShopId,
    ...
    ).rejects.toMatchObject({ code: "transfers.same_source_and_destination" });
  });
});
