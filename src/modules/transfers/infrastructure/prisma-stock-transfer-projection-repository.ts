...
export class PrismaStockTransferProjectionRepository implements StockTransferProjectionRepository {
  ...
  public async list(organizationId: string, shopId: string | null): Promise<StockTransferListProjection | null> {
    if (shopId !== null && await this.prisma.shop.count({ where: { id: shopId, organizationId } }) !== 1) return null;
    const rows = await this.prisma.stockTransfer.findMany({
      where: { organizationId, ...(shopId === null ? {} : { OR: [{ sourceShopId: shopId }, { destinationShopId: shopId }] }) },
      include: {
        sourceShop: { select: { name: true } },
        destinationShop: { select: { name: true } },
        lines: { include: { product: { select: { name: true, code: true } } }, orderBy: { id: "asc" } },
      },
      orderBy: [{ createdAt: "desc" }, { id: "desc" }],
    });
    return StockTransferListProjection.create(organizationId, rows.map((row) => ({
      transferId: row.id,
      ...
      status: row.status,
      lines: row.lines.map((line) => ({ productId: line.productId, productName: line.product.name, productCode: line.product.code, quantity: Number(line.quantity) })),
