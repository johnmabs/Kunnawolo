...
type PaymentRow = Readonly<{
  ...
  actorId: string | null;
  paidAt: Date;
  saleCart: { businessReference: string | null };
}>;
function toPayment(row: PaymentRow): SalePayment {
  if (row.saleCart.businessReference === null)
    throw new DomainError(
      "sales.invalid_payment_state",
      "A paid sale needs a business reference.",
    );
  return SalePayment.reconstitute({
    ...
    actorId: row.actorId,
    paidAt: row.paidAt,
    businessReference: row.saleCart.businessReference,
  });
}
...
export class PrismaSalePaymentRepository implements SalePaymentRepository {
  ...
  public async record(payment: SalePayment): Promise<SalePayment> {
    return this.prisma.$transaction(async (tx) => {
      ...
      if (byReference !== null) {
        if (byReference.saleCartId === payment.cartId.value)
          return toPayment(byReference);
        throw new DomainError(
          "sales.payment_reference_taken",
          "The payment reference is already used.",
        );
      }
      const reserved = await tx.saleCart.updateMany({
        where: {
          id: payment.cartId.value,
          organizationId: payment.organizationId.value,
          status: "FINALIZED",
        },
        data: { status: "PAYING" },
      ...
      if (reserved.count !== 1)
        throw new DomainError(
          ...
          "The sale cannot be paid in this organization.",
        );
      const sequence = await tx.salesSequence.upsert({
        where: { organizationId: payment.organizationId.value },
        create: { organizationId: payment.organizationId.value, nextValue: 2 },
      ...
      await tx.salePayment.create({
        data: {
          id: payment.id.value,
          organizationId: payment.organizationId.value,
          saleCartId: payment.cartId.value,
          paymentReference: payment.paymentReference,
          method: payment.method,
          status: "PAID",
          amountMinor: BigInt(payment.amount.amountMinor),
      ...
      await tx.saleCart.update({
        where: { id: payment.cartId.value },
        data: { status: "PAID", businessReference },
      });
