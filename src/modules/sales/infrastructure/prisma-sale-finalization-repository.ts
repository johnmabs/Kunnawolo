...
type FinalizedCartRow = Readonly<{ id: string; organizationId: string; shopId: string; finalizationReference: string | null; finalizedByActorId: string | null; finalizedAt: Date | null; lines: ReadonlyArray<{ id: string; productId: string; productNameSnapshot: string; quantity: { toString(): string }; unitPriceMinor: bigint; unitCostMinor: bigint; currency: string; discountMinor: bigint }> }>;
...
function toFinalization(row: FinalizedCartRow): SaleFinalization {
  ...
  return SaleFinalization.create(Identifier.fromString(row.id), Identifier.fromString(row.organizationId), Identifier.fromString(row.shopId), lines, row.finalizationReference, row.finalizedByActorId, row.finalizedAt);
}
