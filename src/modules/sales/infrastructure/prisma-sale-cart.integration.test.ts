...
import { GetSaleCart } from "../application/get-sale-cart";
import { RemoveSaleLine } from "../application/remove-sale-line";
...
if (url === undefined)
  throw new Error("DATABASE_URL is required for sale cart integration tests.");
const prisma = createPrismaClient(url);
const org = "cart-integration-org";
const otherOrg = "cart-integration-other";
const shop = "cart-integration-shop";
const product = "cart-integration-product";
let next = 0;
const ids = {
  next: () => Identifier.fromString(`cart-integration-id-${++next}`),
};
beforeAll(async () => {
  await prisma.organization.createMany({
    data: [
      { id: org, name: "Ventes", currency: "XOF" },
  ...
  await prisma.shop.upsert({
    where: { id: shop },
    create: { id: shop, organizationId: org, code: "VTE", name: "Boutique Ɛ" },
    update: { isActive: true },
  });
  await prisma.product.upsert({
    where: { id: product },
    create: { id: product, organizationId: org, name: "Nsiirin Ɛ Ɔ ɲ ŋ Fɔ́lɔ" },
    update: { isActive: true },
  });
  await prisma.productPrice.upsert({
    where: { id: "cart-integration-price" },
    create: {
      id: "cart-integration-price",
      organizationId: org,
      productId: product,
...
describe("PrismaSaleCartRepository", () => {
  it("persists the complete Unicode snapshot lifecycle and keeps historical carts readable", async () => {
    ...
    await expect(
      prisma.saleLine.count({ where: { saleCartId: cart.id.value } }),
    ).resolves.toBe(0);
    await expect(
