...
export class SalePayment {
  private constructor(
    ...
    public readonly businessReference: string | null,
  ) {}
  public static record(
    input: Readonly<{
      id: Identifier;
      finalization: SaleFinalization;
    ...
    if (reference.length === 0)
      throw new DomainError(
        "sales.invalid_payment_reference",
        "A payment reference must be non-empty.",
      );
    if (!methods.includes(input.method as PaymentMethod))
      throw new DomainError(
        "sales.invalid_payment_method",
    ...
    if (
      currency === undefined ||
      input.amount.currency !== currency ||
      input.amount.amountMinor !== expected
    )
      throw new DomainError(
        "sales.invalid_payment_amount",
        "The payment must equal the finalized sale total.",
      );
    return new SalePayment(
      input.id,
      input.finalization.organizationId,
      input.finalization.cartId,
      reference,
  ...
  public numbered(businessReference: string): SalePayment {
    return new SalePayment(
      this.id,
      this.organizationId,
      this.cartId,
