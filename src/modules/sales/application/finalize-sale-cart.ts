...
export class FinalizeSaleCart {
  ...
  public async execute(input: Readonly<{ organizationId: string; cartId: string; reference: string; actorId: string | null; underCostReason?: string | null }>) {
    ...
    const finalization = cart.finalize(reference, input.actorId, this.clock.now(), input.underCostReason);
