...
export class FinalizeSaleCart {
  ...
  public async execute(
    ...
    const existing = await this.finalizations.findByReference(
    ...
    if (existing !== null) {
      if (existing.cartId.value !== input.cartId)
        throw new DomainError(
          "sales.finalization_reference_taken",
          "The finalization reference is already used.",
        );
      return existing;
    }
    const cart = await new GetSaleCart(this.carts).execute(input);
    const finalization = cart.finalize(
      reference,
      input.actorId,
      this.clock.now(),
      input.underCostReason,
    );
