...
export class SaveSaleLine {
  public constructor(
    private readonly scope: SalesScope,
    private readonly carts: SaleCartRepository,
  ...
  public async execute(
    ...
    if (
      ...
      !cart.lines.some(({ id }) => id.value === input.lineId)
    )
      throw new DomainError(
        "sales.line_not_found",
        "The line does not belong to this draft cart.",
      );
    const snapshot = await this.scope.findActiveProductSnapshot(
    ...
    if (snapshot === null)
      throw new DomainError(
        ...
        "The active priced product does not belong to this organization.",
      );
    const line = SaleLine.create({
      id:
        input.lineId === undefined
          ? this.ids.next()
          : Identifier.fromString(input.lineId),
    ...
    await this.carts.saveLine(input.organizationId, input.cartId, line, {
      organizationId: input.organizationId,
      actorId: input.actorId,
      action:
        input.lineId === undefined ? "sale_line.added" : "sale_line.updated",
    });
    return line;
  }
}
