...
class Carts implements SaleCartRepository {
  public readonly cart = SaleCart.draft(
  ...
  public async find(org: string, id: string) {
    return org === "org" && id === "cart" ? this.cart : null;
  }
...
class Finalizations implements SaleFinalizationRepository {
  public saved: SaleFinalization | null = null;
  public async findByReference(org: string, reference: string) {
    return this.saved?.organizationId.value === org &&
      this.saved.reference === reference
      ? this.saved
      : null;
  }
  public async findByCartId(org: string, cartId: string) {
    return this.saved?.organizationId.value === org &&
      this.saved.cartId.value === cartId
      ? this.saved
      : null;
  }
  public async commit(value: SaleFinalization) {
    this.saved = value;
  }
}
describe("FinalizeSaleCart", () => {
  it("finalizes once and returns the same decision on retry", async () => {
    const finalizations = new Finalizations();
    const useCase = new FinalizeSaleCart(new Carts(), finalizations, {
