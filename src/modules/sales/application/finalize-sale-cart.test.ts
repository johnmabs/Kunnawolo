...
class Finalizations implements SaleFinalizationRepository { public saved: SaleFinalization | null = null; public async findByReference(org: string, reference: string) { return this.saved?.organizationId.value === org && this.saved.reference === reference ? this.saved : null; } public async findByCartId(org: string, cartId: string) { return this.saved?.organizationId.value === org && this.saved.cartId.value === cartId ? this.saved : null; } public async commit(value: SaleFinalization) { this.saved = value; } }
