...
export class RecordSalePayment {
  public constructor(
    ...
    private readonly payments: SalePaymentRepository,
  ...
  public async execute(
    ...
    const existing = await this.payments.findByReference(
    ...
    const finalization = await this.finalizations.findByCartId(
      input.organizationId,
      input.cartId,
    );
    if (finalization === null)
      throw new DomainError(
        ...
        "The sale is not finalized in this organization.",
      );
    const payment = SalePayment.record({
      ...
      method: input.method,
      amount: Money.fromMinor(input.amountMinor, input.currency),
      actorId: input.actorId,
      paidAt: this.clock.now(),
    });
    return this.payments.record(payment);
  }
}
