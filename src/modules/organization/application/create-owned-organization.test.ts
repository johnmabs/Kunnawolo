...
describe("CreateOwnedOrganization", () => {
  it("creates the organization and its owner membership as one repository operation", async () => {
    let saved:
      | Parameters<OrganizationOnboardingRepository["createWithOwner"]>[0]
      | null = null;
    ...
    const result = await new CreateOwnedOrganization(repository, {
      next: () => Identifier.fromString(values.shift()!),
    }).execute({
      name: "ASTU SARL",
      currency: "XOF",
    ...
    expect(saved).toMatchObject({
      ...
      ownerUserAccountId: "user-id",
      initialShop: { code: "CENTRE", name: "Centre-ville" },
    });
  });
});
