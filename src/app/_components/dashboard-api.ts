import { DashboardApiError, type Dashboard, type DashboardAccess, type DashboardFilters } from "./dashboard-types";
...
const messages: Readonly<Record<string, string>> = { "reporting.read_forbidden": "Seuls les propriétaires et responsables autorisés peuvent consulter le reporting.", "reporting.invalid_date_range": "La période sélectionnée est invalide.", "reporting.shop_not_found": "La boutique sélectionnée n’existe plus.", "security.invalid_api_key": "La clé d’accès est invalide ou révoquée." };
export function dashboardErrorMessage(error: unknown) { return error instanceof DashboardApiError ? messages[error.code] ?? `Une erreur est survenue (${error.code}).` : "Une erreur inattendue est survenue."; }
