...
export async function signupAction(_previous: SignupState, formData: FormData): Promise<SignupState> {
  ...
  if (!databaseUrl) return { error: "Le service d’inscription est momentanément indisponible." };
  const displayName = formData.get("displayName"); const email = formData.get("email"); const password = formData.get("password");
  if (typeof displayName !== "string" || typeof email !== "string" || typeof password !== "string") return { error: "Vérifiez les informations du formulaire." };
  const prisma = createPrismaClient(databaseUrl);
  try {
    const repository = new PrismaWebAuthenticationRepository(prisma);
    const account = await new RegisterWithPassword(repository, new NodePasswordHasher(), new UuidIdentifierGenerator()).execute({ displayName, email, password });
