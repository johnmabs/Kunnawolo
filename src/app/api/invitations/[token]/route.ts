...
export async function GET(_request: Request, context: Context) {
  ...
  try {
    ...
    const details = await new GetMembershipInvitation(new PrismaMembershipInvitationRepository(prisma), new NodeOpaqueToken(), new SystemClock()).execute(token);
    return NextResponse.json({ email: details.account.email, displayName: details.account.displayName, organizationName: details.organizationName, expiresAt: details.invitation.expiresAt.toISOString(), requiresPassword: !details.hasCredential });
  } catch (error) { return apiErrorResponse(error, "auth.invitation_read_failed"); }
...
export async function POST(request: Request, context: Context) {
  ...
  try {
    ...
    try { authenticatedUserAccountId = (await authenticateWebRequest(prisma)).id.value; } catch { /* A new invited account is not authenticated yet. */ }
    const result = await new AcceptMembershipInvitation(new PrismaMembershipInvitationRepository(prisma), new NodeOpaqueToken(), new NodePasswordHasher(), new SystemClock()).execute({ token, password: typeof input.password === "string" ? input.password : null, authenticatedUserAccountId });
  ...
  } catch (error) { return apiErrorResponse(error, "auth.invitation_acceptance_failed"); }
  finally { await prisma.$disconnect(); }
}
