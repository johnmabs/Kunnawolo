...
type PricingRequest = Readonly<{ organizationId?: string; referenceCostMinor?: number; salePriceMinor?: number; currency?: string; reference?: string }>;

export async function POST(request: Request, route: Readonly<{ params: Promise<{ productId: string }> }>) {
  const input = await request.json() as PricingRequest;
  const organizationId = input.organizationId?.trim();
  const currency = input.currency?.trim();
  const reference = input.reference?.trim();
  if (!organizationId || !currency || !reference || !Number.isSafeInteger(input.referenceCostMinor) || !Number.isSafeInteger(input.salePriceMinor)) return NextResponse.json({ code: "catalog.invalid_pricing_request" }, { status: 400 });
  ...
  try {
    ...
    const pricing = await new SetProductPricing(new PrismaProductRepository(prisma), new PrismaProductPricingRepository(prisma), new UuidIdentifierGenerator(), new SystemClock()).execute({ organizationId, productId, referenceCostMinor: input.referenceCostMinor as number, salePriceMinor: input.salePriceMinor as number, currency, reference, actorId: access.actorId });
