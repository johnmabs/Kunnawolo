...
export async function GET(request: Request, route: Readonly<{ params: Promise<{ productId: string }> }>) {
  const scope = await context(request);
  if (scope === null) return NextResponse.json({ code: "catalog.invalid_product_request" }, { status: 400 });
  const databaseUrl = process.env.DATABASE_URL;
  if (databaseUrl === undefined) return NextResponse.json({ code: "catalog.unavailable" }, { status: 503 });
  const prisma = createPrismaClient(databaseUrl);
  try {
    await authenticateApiRequest(prisma, request.headers.get("authorization"), scope.organizationId);
    ...
    const [product, pricing, organization] = await Promise.all([
      new GetProduct(products).execute({ organizationId: scope.organizationId, productId }),
      new PrismaProductPricingRepository(prisma).findCurrent(scope.organizationId, productId),
    ...
    return NextResponse.json({ ...productDto(product), pricing: pricingDto(pricing, organization.currency) });
  } catch (error) {
    return apiErrorResponse(error, "catalog.product_detail_failed");
  } finally { await prisma.$disconnect(); }
}

type LifecycleRequest = Readonly<{ organizationId?: string; isActive?: boolean }>;

export async function PATCH(request: Request, route: Readonly<{ params: Promise<{ productId: string }> }>) {
  const input = await request.json() as LifecycleRequest;
  const organizationId = input.organizationId?.trim();
  if (!organizationId || typeof input.isActive !== "boolean") return NextResponse.json({ code: "catalog.invalid_lifecycle_request" }, { status: 400 });
  const databaseUrl = process.env.DATABASE_URL;
  if (databaseUrl === undefined) return NextResponse.json({ code: "catalog.unavailable" }, { status: 503 });
  const prisma = createPrismaClient(databaseUrl);
  try {
    ...
    const product = input.isActive
      ? await new ActivateProduct(products).execute({ organizationId, productId, actorId: access.actorId })
      : await new DeactivateProduct(products).execute({ organizationId, productId, actorId: access.actorId });
