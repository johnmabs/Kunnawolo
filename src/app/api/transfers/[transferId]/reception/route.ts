...
type ReceptionRequest = Readonly<{
...
export async function POST(
  ...
  if (!organizationId || !reference)
    return NextResponse.json(
      { code: "transfers.invalid_reception_request" },
      { status: 400 },
    );
  ...
  try {
    ...
    await new ReceiveStockTransfer(
      new PrismaStockTransferRepository(prisma),
    ...
    return apiErrorResponse(error, "transfers.reception_failed");
  } finally {
