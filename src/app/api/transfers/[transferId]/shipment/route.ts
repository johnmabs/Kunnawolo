...
export async function POST(
  ...
  if (!organizationId || !reference)
    return NextResponse.json(
      { code: "transfers.invalid_shipment_request" },
      { status: 400 },
    );
  ...
  try {
    ...
    return apiErrorResponse(error, "transfers.shipment_failed");
  } finally {
