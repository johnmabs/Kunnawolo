...
export async function PUT(
  request: Request,
  context: Readonly<{ params: Promise<{ transferId: string }> }>,
) {
  const input = (await request.json()) as LineRequest;
  const organizationId = input.organizationId?.trim();
  const productId = input.productId?.trim();
  if (!organizationId || !productId || !Number.isFinite(input.quantity))
    return NextResponse.json(
      { code: "transfers.invalid_line_request" },
      { status: 400 },
    );
  const databaseUrl = process.env.DATABASE_URL;
  if (databaseUrl === undefined)
    return NextResponse.json(
      { code: "transfers.unavailable" },
      { status: 503 },
    );
  ...
  try {
    ...
    if (draft === null)
      return NextResponse.json(
        { code: "transfers.draft_not_found" },
        { status: 404 },
      );
    await new PrismaTransfersDispatchAuthorization(prisma).authorize(
      organizationId,
      draft.sourceShopId.value,
      access.actorId,
    );
    await new SaveStockTransferLine(
      new PrismaTransferScope(prisma),
      transfers,
    ...
    return apiErrorResponse(error, "transfers.line_save_failed");
  } finally {
