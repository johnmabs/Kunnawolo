...
export async function GET(request: Request) {
  const organizationId = new URL(request.url).searchParams
    ...
    ?.trim();
  if (!organizationId)
    return NextResponse.json({ code: "shop.invalid_request" }, { status: 400 });
  const databaseUrl = process.env.DATABASE_URL;
  if (databaseUrl === undefined)
    return NextResponse.json({ code: "shop.unavailable" }, { status: 503 });
  const prisma = createPrismaClient(databaseUrl);
  try {
    const { membership } = await access(prisma, organizationId);
    if (membership?.status !== "ACTIVE")
      return NextResponse.json(
        { code: "shop.list_forbidden" },
        { status: 403 },
      );
    const permittedIds =
      membership.role === "OWNER"
        ? undefined
        : membership.shopAssignments.map(({ shopId }) => shopId);
    ...
    return apiErrorResponse(error, "shop.list_failed");
  } finally {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return apiErrorResponse(error, "shop.creation_failed");
  } finally {
    await prisma.$disconnect();
  }
}
