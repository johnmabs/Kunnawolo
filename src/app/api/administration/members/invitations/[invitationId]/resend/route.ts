...
import { assertTrustedOrigin, authenticateWebRequest } from "@/app/api/auth/_shared/web-session-access";

export async function POST(request: Request, context: { params: Promise<{ invitationId: string }> }) {
  const databaseUrl = process.env.DATABASE_URL;
  if (!databaseUrl) return NextResponse.json({ code: "iam.unavailable" }, { status: 503 });
  const prisma = createPrismaClient(databaseUrl);
  try {
    assertTrustedOrigin(request);
    const account = await authenticateWebRequest(prisma);
    const input = await request.json() as { organizationId?: unknown };
    if (typeof input.organizationId !== "string") return NextResponse.json({ code: "iam.invalid_invitation_request" }, { status: 400 });
    const { invitationId } = await context.params;
    const opaqueTokens = new NodeOpaqueToken();
    const issued = await new ResendMembershipInvitation(new PrismaMembershipInvitationRepository(prisma), new UuidIdentifierGenerator(), opaqueTokens, opaqueTokens, new SystemClock(), process.env.APP_URL ?? new URL(request.url).origin).execute({ organizationId: input.organizationId, invitationId, actorId: account.id.value });
    ...
    return NextResponse.json({ expiresAt: issued.invitation.expiresAt.toISOString(), delivery, ...(configured?.mode === "manual" ? { acceptanceUrl: issued.acceptanceUrl } : {}) });
  } catch (error) {
