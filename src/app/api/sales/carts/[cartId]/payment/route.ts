...
export async function POST(
  request: Request,
  context: Readonly<{ params: Promise<{ cartId: string }> }>,
) {
  ...
  if (
    ...
    return NextResponse.json(
      { code: "sales.invalid_payment_request" },
      { status: 400 },
    );
  ...
  try {
    ...
    const finalization = await finalizations.findByCartId(
      organizationId,
      cartId,
    );
    ...
    await new PrismaWorkspacePreferenceAuthorization(prisma).authorize(
      organizationId,
      access.actorId,
      finalization.shopId.value,
    );
    const payment = await new RecordSalePayment(
      finalizations,
