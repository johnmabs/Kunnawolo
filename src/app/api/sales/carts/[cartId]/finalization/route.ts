...
export async function POST(
  ...
  if (!organizationId || !reference)
    return NextResponse.json(
      { code: "sales.invalid_finalization_request" },
      { status: 400 },
    );
  ...
  try {
    ...
    const cart = await new GetSaleCart(carts).execute({
      organizationId,
      cartId,
    });
    ...
    const finalization = await new FinalizeSaleCart(
      carts,
      ...
      underCostReason: input.underCostReason,
    });
