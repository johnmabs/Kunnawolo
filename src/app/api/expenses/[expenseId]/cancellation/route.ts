...
type CancellationRequest = Readonly<{
...
export async function POST(
  request: Request,
  context: Readonly<{ params: Promise<{ expenseId: string }> }>,
) {
  const input = (await request.json()) as CancellationRequest;
  const organizationId = input.organizationId?.trim();
  const reference = input.reference?.trim();
  const reason = input.reason?.trim();
  if (!organizationId || !reference || !reason)
    return NextResponse.json(
      { code: "expenses.invalid_cancellation_request" },
      { status: 400 },
    );
  ...
  try {
    ...
    if (expense === null)
      return NextResponse.json(
        { code: "expenses.expense_not_found" },
        { status: 404 },
      );
    await new PrismaExpenseReadAuthorization(prisma).authorize(
      organizationId,
      expense.shopId,
      access.actorId,
    );
    const cancellation = await new CancelExpense(
    ...
    return apiErrorResponse(error, "expenses.cancellation_failed");
  } finally {
