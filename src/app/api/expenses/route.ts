...
export async function GET(request: Request) {
  ...
  if (!organizationId) return NextResponse.json({ code: "expenses.invalid_list_request" }, { status: 400 });
  ...
  try {
    ...
    const items = await new ListExpenses(new PrismaExpenseConsultationRepository(prisma), new PrismaExpenseReadAuthorization(prisma)).execute({
      organizationId,
    ...
    return NextResponse.json({ items: items.map(({ expense, categoryName, cancellation }) => ({
      id: expense.id.value,
      ...
      cancellation: cancellation === null ? null : { reference: cancellation.reference, reason: cancellation.reason, cancelledAt: cancellation.cancelledAt },
    })) });
  } catch (error) { return apiErrorResponse(error, "expenses.list_failed"); }
