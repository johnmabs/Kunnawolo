...
export async function PUT(request: Request) {
  ...
  try {
    ...
    if (idempotencyKey === null) return NextResponse.json({ code: "workspace.idempotency_key_required" }, { status: 400 });
    const preferences = new PrismaWorkspacePreferenceRepository(prisma);
    const preference = await new SaveWorkspacePreferenceIdempotently(preferences, preferences, new PrismaWorkspacePreferenceAuthorization(prisma)).execute({ organizationId: input.organizationId, actorId: access.actorId, shopId: input.shopId, isCompact: input.isCompact, idempotencyKey });
