...
export async function authenticateWebRequest(prisma: PrismaClient) {
  return new AuthenticateWebSession(new PrismaWebAuthenticationRepository(prisma), new NodeOpaqueToken(), new SystemClock()).execute(await readSessionToken());
}

export function assertTrustedOrigin(request: Request): void {
  const origin = request.headers.get("origin");
  const expectedOrigin = process.env.APP_URL ? new URL(process.env.APP_URL).origin : new URL(request.url).origin;
  if (origin !== expectedOrigin) throw new DomainError("security.invalid_origin", "The request origin is not trusted.");
}
