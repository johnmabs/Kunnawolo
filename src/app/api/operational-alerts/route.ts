...
export async function GET(request: Request) {
  ...
  if (organizationId === null) return NextResponse.json({ code: "observability.invalid_alert_request" }, { status: 400 });
  ...
  try {
    ...
    const code = error instanceof Error && "code" in error ? String(error.code) : "observability.alerts_failed";
