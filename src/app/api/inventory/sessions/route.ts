...
export async function GET(request: Request) {
  ...
  if (!organizationId || !shopId) return NextResponse.json({ code: "inventory.invalid_session_request" }, { status: 400 });
  ...
  try {
    ...
    const projection = await new ListInventorySessions(new PrismaInventoryProjectionRepository(prisma)).execute({ organizationId, shopId });
