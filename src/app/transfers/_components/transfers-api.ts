...
async function request<T>(
  ...
  if (!response.ok)
    throw new TransferApiError(body.code ?? "transfers.unknown_error");
  return body;
}
...
export function searchTransferProducts(
  access: TransferAccess,
  query: string,
): Promise<TransferProductSearch> {
  const search = new URLSearchParams({
    organizationId: access.organizationId,
    query,
  });
...
export async function saveTransferLine(
  access: TransferAccess,
  transferId: string,
  productId: string,
  quantity: number,
