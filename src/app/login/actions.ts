...
export async function loginAction(_previous: LoginState, formData: FormData): Promise<LoginState> {
  ...
  if (!databaseUrl) return { error: "Le service de connexion est momentanément indisponible." };
  const email = formData.get("email"); const password = formData.get("password"); const requestedNext = formData.get("next");
  if (typeof email !== "string" || typeof password !== "string") return { error: "Vérifiez les informations du formulaire." };
  const prisma = createPrismaClient(databaseUrl);
  try {
    const repository = new PrismaWebAuthenticationRepository(prisma);
    const account = await new LoginWithPassword(repository, new NodePasswordHasher()).execute({ email, password });
  ...
  const destination = typeof requestedNext === "string" && requestedNext.startsWith("/") && !requestedNext.startsWith("//") ? requestedNext : "/";
