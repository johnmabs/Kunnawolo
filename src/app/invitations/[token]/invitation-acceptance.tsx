...
import { acceptInvitationAction, type InvitationAcceptanceState } from "./actions";

type Details = Readonly<{ email: string; displayName: string; organizationName: string; expiresAt: string; requiresPassword: boolean }>;

export function InvitationAcceptance({ token, details }: Readonly<{ token: string; details: Details | null }>) {
  const actionWithToken = acceptInvitationAction.bind(null, token);
  const [state, action, pending] = useActionState(actionWithToken, { error: null } satisfies InvitationAcceptanceState);
  if (details === null) return <div className="grid gap-4"><AuthError message="Ce lien d’invitation est invalide, expiré ou déjà utilisé." /><Button asChild variant="secondary"><Link href="/login">Retour à la connexion</Link></Button></div>;
  return (
    <form action={action} className="grid gap-5">
      ...
      <Button className="w-full" isLoading={pending} size="lg" type="submit">Rejoindre {details.organizationName}</Button>
    </form>
  );
}
