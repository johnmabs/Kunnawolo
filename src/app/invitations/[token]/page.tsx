...
export const metadata: Metadata = {
  title: "Accepter l’invitation · Astu Sales",
};
export default async function InvitationPage({
  ...
  if (databaseUrl) {
    ...
    try {
      const invitation = await new GetMembershipInvitation(
      ...
      details = {
        ...
        displayName: invitation.account.displayName,
        organizationName: invitation.organizationName,
        expiresAt: invitation.invitation.expiresAt.toISOString(),
        requiresPassword: !invitation.hasCredential,
      };
    } catch {
      /* Invalid invitations render a stable error state. */
    } finally {
  ...
  return (
    <AuthLayout
      description="Vérifiez les informations ci-dessous avant de rejoindre l’espace de travail."
      eyebrow="Invitation"
      title="Rejoindre une organisation"
    >
      <InvitationAcceptance details={details} token={token} />
    </AuthLayout>
  );
}
