...
export async function acceptInvitationAction(
  token: string,
  _previous: InvitationAcceptanceState,
  formData: FormData,
): Promise<InvitationAcceptanceState> {
  const databaseUrl = process.env.DATABASE_URL;
  if (!databaseUrl)
    return { error: "Le service d’invitation est momentanément indisponible." };
  ...
  try {
    ...
    try {
      ...
      /* A new invited account has no session yet. */
    }
    ...
    else if (code === "auth.password_too_short")
      return {
        error: "Choisissez une phrase de passe d’au moins 15 caractères.",
      };
    else
      return {
        error:
          "L’invitation n’a pas pu être acceptée. Le lien est peut-être expiré.",
      };
  } finally {
