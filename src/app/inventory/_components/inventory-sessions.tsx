...
export function InventorySessions({
  access,
}: Readonly<{ access: InventoryAccess }>) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      setSessions(await listInventorySessions(access));
    } catch (loadError) {
  ...
  useEffect(() => {
    let active = true;
    void listInventorySessions(access)
      .then((result) => {
  ...
  if (!sessions || sessions.items.length === 0)
    return (
      <EmptyState
        description="Aucune session d’inventaire n’a été ouverte dans cette boutique."
        ...
        >
          <div>
            ...
            <p className="mt-2 text-sm text-text-secondary">
              {session.countedLineCount}/{session.totalLineCount} produits
              comptés · {session.progressPercentage}%
            </p>
            {session.status !== "OPEN" ? (
              <p className="mt-1 text-sm text-text-secondary">
                {session.discrepancyLineCount} écarts ·{" "}
                {session.discrepancyQuantity} unités
              </p>
            ) : null}
          </div>
          <Button
            disabled
            title="BACKEND GAP: InventorySessionDetailProjection"
            variant="secondary"
          >
            {session.status === "OPEN" ? "Continuer" : "Consulter"}
          </Button>
        </article>
      ))}
    </div>
