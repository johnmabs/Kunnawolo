...
export function ReceiptDialog({ busy, detail, onConfirm, onOpenChange, open }: Readonly<{ busy: boolean; detail: StockDetail; onConfirm: (quantity: number, reference: string) => void; onOpenChange: (open: boolean) => void; open: boolean }>) {
  ...
  return (
    <Dialog onOpenChange={onOpenChange} open={open}><DialogContent>
      ...
      <DialogFooter><Button disabled={busy} onClick={() => onOpenChange(false)} variant="secondary">Annuler</Button><Button disabled={!valid || busy} isLoading={busy} onClick={() => onConfirm(parsedQuantity, reference)}>Réceptionner</Button></DialogFooter>
