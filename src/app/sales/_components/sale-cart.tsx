...
export function SaleCart({ busyLineId, cart, onDiscount, onFinalize, onQuantityChange, onRemove }: SaleCartProps) {
  const currency = cart.lines[0]?.currency;

  return (
    <section aria-labelledby="sale-cart-title" className="flex min-h-0 flex-col border-t border-border bg-surface lg:border-l lg:border-t-0">
      <div className="flex items-center justify-between border-b border-border p-4 sm:p-6">
        <h2 className="text-base font-semibold text-text-primary" id="sale-cart-title">Panier</h2>
        <span className="text-sm text-text-secondary">{cart.lines.length} {cart.lines.length > 1 ? "articles" : "article"}</span>
      </div>
      <div className="flex-1 overflow-y-auto p-4 sm:p-6">
        {cart.lines.length === 0 ? <EmptyState description="Recherchez un produit pour commencer." title="Votre panier est vide" /> : (
