...
export function UnderCostDialog({
  lines,
  onContinue,
  onOpenChange,
  open,
}: UnderCostDialogProps) {
    ...
    <Dialog onOpenChange={handleOpenChange} open={open}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogTitle>Vente sous le coût de référence</DialogTitle>
          <DialogDescription>
            La remise place{" "}
            {lines.length > 1 ? `${lines.length} lignes` : "cette ligne"} sous
            le coût enregistré.
          </DialogDescription>
        </DialogHeader>
          ...
          {lines.map((line) => (
            ...
            >
              ...
              <dl className="mt-3 grid grid-cols-2 gap-2 text-sm">
                <dt className="text-text-secondary">Prix après remise</dt>
                ...
                <dd className="text-right font-medium">
                  {formatMoney(
                    line.unitCostMinor * line.quantity,
                    line.currency,
                  )}
                </dd>
                <dt className="text-text-secondary">Écart</dt>
                <dd className="text-right font-semibold text-danger">
                  {formatMoney(
                    line.lineTotalMinor - line.unitCostMinor * line.quantity,
                    line.currency,
                  )}
                </dd>
              </dl>
          ...
          <Field label="Justification" name="under-cost-reason" required>
            {({ controlId }) => (
              <Textarea
                ...
                placeholder="Expliquez pourquoi cette vente doit continuer…"
                required
                value={reason}
              />
            )}
