...
export function DiscountDialog({
  ...
  return (
    <DiscountDialogForm
      busy={busy}
      key={`${line.id}:${line.discountMinor}`}
      line={line}
      onApply={onApply}
...
function DiscountDialogForm({
  busy,
  line,
  onApply,
  onOpenChange,
  open,
}: Omit<DiscountDialogProps, "line"> & Readonly<{ line: SaleLineDto }>) {
    ...
    <Dialog onOpenChange={onOpenChange} open={open}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            La remise s’applique uniquement à cette ligne.
          </DialogDescription>
        </DialogHeader>
        <div className="mt-6 grid gap-4">
          <div>
            <p className="text-xs text-text-secondary">Produit</p>
            <p className="font-semibold">{line.productName}</p>
          </div>
          <dl className="grid grid-cols-2 gap-3 rounded-lg bg-surface-subtle p-4 text-sm">
            <dt className="text-text-secondary">Prix</dt>
            <dd className="text-right font-medium">
              {formatMoney(line.unitPriceMinor, line.currency)}
            </dd>
            <dt className="text-text-secondary">Quantité</dt>
            <dd className="text-right font-medium">{line.quantity}</dd>
          </dl>
          <Field
            error={
              !valid && discount.length > 0
                ? "La remise doit être comprise entre zéro et le montant de la ligne."
                : undefined
            }
            label="Remise totale"
            name="line-discount"
          ...
          <div className="rounded-lg border border-border p-4">
            <p className="text-xs text-text-secondary">Nouveau total</p>
            <p className="mt-1 text-xl font-semibold tabular-nums">
              {valid
                ? formatMoney(grossMinor - discountMinor, line.currency)
                : "—"}
            </p>
          </div>
        </div>
