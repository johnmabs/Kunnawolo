...
export function ProductSearch({
  ...
  return (
    <section
      ...
      className="flex min-h-0 flex-col max-lg:max-h-[55dvh]"
    >
      <div className="border-b border-border p-4 sm:p-6">
        ...
        <p className="mt-2 text-xs text-text-secondary">
          Saisissez au moins deux caractères. La projection des prix de vente
          reste un BACKEND GAP confirmé.
        </p>
      </div>
      <div className="flex-1 overflow-y-auto p-4 sm:p-6">
        {normalizedQuery.length < 2 ? (
          <EmptyState
            description="Recherchez un produit par son nom, son code ou son code-barres."
            title="Commencez votre recherche"
          />
        ) : null}
        {loading ? (
          <div className="grid gap-3" aria-label="Chargement des produits">
        ...
        {!loading && error ? (
          <ErrorState description={error} onRetry={() => void runSearch()} />
        ) : null}
        {!loading &&
        !error &&
        normalizedQuery.length >= 2 &&
        products.length === 0 ? (
          <EmptyState
            description="Aucun résultat ne correspond à cette recherche."
        ...
        {!loading && !error && products.length > 0 ? (
          <ul className="grid gap-3">
            {products.map((product) => (
              <li
                className="flex min-h-24 items-center justify-between gap-4 rounded-lg border border-border bg-surface p-4"
                key={product.id}
              >
                <div className="min-w-0">
                  ...
                  <p className="mt-1 truncate text-xs text-text-secondary">
                    {product.code ?? product.barcode ?? "Sans code"}
                  </p>
                  <p className="mt-2 text-sm font-medium text-text-secondary">
                    Prix chargé lors de l’ajout
                  </p>
                </div>
                <Button
