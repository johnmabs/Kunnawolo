...
export type CreateShopState = Readonly<{
  error: string | null;
  created: boolean;
  revision: number;
}>;
...
export async function createShopAction(
  ...
  try {
    ...
    if (membership?.status !== "ACTIVE" || membership.role !== "OWNER")
      return {
        error: "Vous n’êtes pas autorisé à créer une boutique.",
        created: false,
        revision: previous.revision + 1,
      };
    await new CreateShop(
      new PrismaShopRepository(prisma),
      new PrismaAuditLog(prisma),
      new UuidIdentifierGenerator(),
    ).execute({ organizationId, name, code, actorId: account.id.value });
