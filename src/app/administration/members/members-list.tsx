...
type DeliveryStatus =
  "PENDING" | "PROCESSING" | "FAILED" | "SENT" | "CANCELLED";
export type Member = Readonly<{
  ...
  invitationDeliveryStatus: DeliveryStatus | null;
...
const deliveries: Readonly<Record<DeliveryStatus, string>> = {
...
function DeliveryDetails({ member }: Readonly<{ member: Member }>) {
  if (member.status !== "INVITED" || member.invitationDeliveryStatus === null)
    return null;
  return (
    <p
      className={
    ...
    >
      {deliveries[member.invitationDeliveryStatus]}
      {member.invitationDeliveryStatus === "FAILED"
        ? ` · ${member.invitationDeliveryAttempts} tentative${member.invitationDeliveryAttempts > 1 ? "s" : ""}`
        : ""}
    </p>
  );
}
...
export function MembersList({
  ...
  if (items.length === 0)
    return (
      <EmptyState
        description="Aucun membre n’existe encore dans cette organisation."
        title="Aucun membre"
      />
    );
  return (
              ...
              {items.map((member) => (
                <tr className="border-t border-border" key={member.id}>
                  ...
                  <td className="px-4 py-4">
                    ...
                    <DeliveryDetails member={member} />
                  </td>
                  <td className="px-4 py-4">
                    {roles[member.role] ?? member.role}
                  </td>
                  <td className="px-4 py-4 text-right">
                    {member.status === "INVITED" && member.invitationId ? (
                      <ResendInvitationForm
                        failed={member.invitationDeliveryStatus === "FAILED"}
