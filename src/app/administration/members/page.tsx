...
export default async function MembersPage() {
  ...
  let items = null;
  if (context && process.env.DATABASE_URL && context.role === "OWNER") {
    const prisma = createPrismaClient(process.env.DATABASE_URL);
    try {
      const result = await new ListMembers(
        new PrismaMembershipConsultationRepository(prisma),
      ).execute(context.organization.id);
  ...
  return (
    <AdministrationPage
      description="Invitez de nouveaux utilisateurs et consultez les membres de l’organisation."
      title="Membres"
    >
      <div className="grid gap-6">
        <InviteMemberActionForm />
        <MembersList items={items ?? []} key={context?.organization.id} />
      </div>
    </AdministrationPage>
  );
}
